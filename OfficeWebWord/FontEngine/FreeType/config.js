...
function _FT_Common()
{
    ...
    this.UintToInt = function(v){
        return (v>this.m_i)?v-this.a_i:v;
    }
    this.UShort_To_Short = function(v){
        return (v>this.m_s)?v-this.a_s:v;
    }
    this.IntToUInt = function(v){
        return (v<0)?v+this.a_i:v;
    }
    this.Short_To_UShort = function(v){
        return (v<0)?v+this.a_s:v;
    }
...
var _dump_for_release_ = false;
// for release version
if (_dump_for_release_)
{
    console.log("----------------------------------------------------");
    for (var i in FT_Common)
    {
		if (true === FT_Common[i])
		{
			console.log("_dict[\"FT_Common." + i + "\"] = \"true\";");
		}
		else if (false === FT_Common[i])
		{
			console.log("_dict[\"FT_Common." + i + "\"] = \"false\";");
		}
		else
		{	
			...
			if (tof == "number")
			{
				console.log("_dict[\"FT_Common." + i + "\"] = \"" + FT_Common[i] + "\";");
			}
		}
    }
    console.log("----------------------------------------------------");
