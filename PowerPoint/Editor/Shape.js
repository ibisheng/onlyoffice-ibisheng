...
function CShapeLocks()
{
    ...
    {
        var _copy  = new CShapeLocks();
        _copy.noAdjustHandles = this.noAdjustHandles;
        _copy.noChangeArrowhe = this.noChangeArrowhe;
        _copy.noChangeAspect =  this.noChangeAspect;
        _copy.noChangeShapeTy = this.noChangeShapeTy;
        _copy.noEditPoints =  this.noEditPoints;
        _copy.noGrp  =  this.noGrp;
        _copy.noMove =  this.noMove;
        _copy.noResize  =  this.noResize;
        _copy.noRot =  this.noRot;
        _copy.noSelect =  this.noSelect;
        _copy.noTextEdit =   this.noTextEdit;
        return _copy;
    }
}
...
CShape.prototype =
{
    ...
    Get_SelectedText: function(bClear)
    {
        ...
        {
            return this.txBody.content.Get_SelectedText(bClear);
        }
        return null;
    },
    Update_CursorType: function(x, y)
    {
        ...
        if(this.txBody )
        {
            vx -=this.pH;
            vy -= this.pV;
            if(this.txBody.content2 != null && this.txBody.content.Is_Empty())
            {

                this.txBody.content2.Update_CursorType(vx  - this.txBody.l2, vy - this.txBody.t2);
            }
            else
            {
                this.txBody.content.Update_CursorType(vx - this.txBody.l, vy -  this.txBody.t);
            }
        }
        else
        {
            this.Container.DrawingDocument.SetCursorType("pointer");
        }
    },
    ...
    hitToHyperlink: function(x, y)
    {
        ...
        if(this.txBody && this.txBody.content)
        {
            vx -= (this.pH + this.txBody.l);
            vy -= (this.pV + this.txBody.t);
            return this.txBody.content.hitToHyperlink(vx, vy);
        }
        return false;
    },
    ...
    Hyperlink_CanAdd : function()
    {
        if(this.txBody != null && this.txBody.content != null)
        {
            return this.txBody.content.Hyperlink_CanAdd();
        }
        return false;
    },
    ...
    getPhIndex: function()
    {
        if(this.isPlaceholder())
        {
            return this.nvSpPr.nvPr.ph.idx;
        }
        else
        {
            return null;
    ...
    check_bounds: function(checker)
    {
        ...
        else
        {
            checker._s();
            checker._m(0, 0);
            checker._l(this.ext.cx, 0);
            checker._l(this.ext.cx, this.ext.cy);
            checker._l(0, this.ext.cy);
            checker._z();
            checker._e();
        }
    },
    ...
    DrawAdj: function(graphics,zoom)
    {
        if(this.geometry && this.ext.cx>min_size&&this.ext.cy>min_size)
        {
            ...
            {
                diamond(graphics, this.geometry.ahXYLst[i].posX, this.geometry.ahXYLst[i].posY, d);
            }
            for(i=0; i<this.geometry.ahPolarLst.length; i++)
            {
                diamond(graphics, this.geometry.ahPolarLst[i].posX, this.geometry.ahPolarLst[i].posY, d);
            }
            graphics.reset();
        }
            ...
            for(var i=0; i<pathLst.length; i++)
            {
                ...
                for(var j=0; j<path.ArrPathCommand.length; j++)
                {
                    ...
                    switch(cmd.id)
                    {
                        ...
                        case arcTo:
                        {
                            ArcToOnCanvas(context, cmd.stX, cmd.stY, cmd.wR, cmd.hR, cmd.stAng, cmd.swAng);
                            break;
                        }
                        case bezier3:
                        {
                            context.quadraticCurveTo(cmd.X0, cmd.Y0, cmd.X1, cmd.Y1);
                            break;
                        }
                        case bezier4:
                        {
                            context.bezierCurveTo(cmd.X0, cmd.Y0, cmd.X1, cmd.Y1, cmd.X2, cmd.Y2);
                            break;
                        }
                        case close:
                        {
        ...
        for(var i=0; i<pathLst.length; i++)
        {
            ...
            for(var j=0; j<path.ArrPathCommand.length; j++)
            {
                ...
                switch(cmd.id)
                {
                    ...
                    case lineTo:
                    {
                        if(HitInLine(context, vx, vy, lastX, lastY, cmd.X, cmd.Y))
                            return true;
                        lastX=cmd.X;
                        lastY=cmd.Y;
                        break;
                    ...
                    case arcTo:
                    {
                        if(HitToArc(context, vx, vy,  cmd.stX, cmd.stY, cmd.wR, cmd.hR, cmd.stAng, cmd.swAng))
                            return true;
                    ...
                    case bezier3:
                    {
                        if(HitInBezier3(context, vx, vy, lastX, lastY, cmd.X0, cmd.Y0, cmd.X1, cmd.Y1))
                            return true;
                        lastX=cmd.X1;
                        lastY=cmd.Y1;
                        break;
                    ...
                    case bezier4:
                    {
                        if(HitInBezier4(context, vx, vy, lastX, lastY, cmd.X0, cmd.Y0, cmd.X1, cmd.Y1, cmd.X2, cmd.Y2))
                            return true;
                        lastX=cmd.X2;
                        lastY=cmd.Y2;
                        break;
                    ...
                    case close:
                    {
                        if(HitInLine(context, vx, vy, lastX, lastY, beginX, beginY))
                            return true;
                    }
                }
            }
    ...
    {
        return this.txBody && this.txBody.content ? this.txBody.content.canIncreaseIndent(bIncrease) : false;
    },
