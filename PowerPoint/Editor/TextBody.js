...
CTextBody.prototype =
{
                    ...
                    || !isNaN(spacing_scale) && typeof spacing_scale === "number")
                {
                    ...
                    for(var index = 0; index < pars.length; ++index)
                    {
                        ...
                        if(!isNaN(spacing_scale) && typeof spacing_scale === "number")
                        {
                            ...
                            var spacing2 = parg.CompiledPr.Pr.ParaPr;
                        ...
                        if(!isNaN(font_scale) && typeof font_scale === "number")
                        {
                            var par_font_size = parg.CompiledPr.Pr.TextPr.FontSize;
                        ...
                        parg.RecalcInfo.Recalc_0_Type = pararecalc_0_All;
                        parg.Recalculate();
                    }

                    if(this.content2 !== null && typeof this.content === "object")
                    {
                        pars = this.content2.Content;
                        for(index = 0; index < pars.length; ++index)
                        {
                            parg = pars[index];
                            if(!isNaN(spacing_scale) && typeof spacing_scale === "number")
                            {
                                spacing = parg.Pr.Spacing;
                                spacing2 = parg.CompiledPr.Pr.ParaPr;
                                spc = (spacing2.Line*spacing_scale);
                                if(!isNaN(spc) && typeof spc === "number")
                                    spacing.Line = spc;
                            ...
                            if(!isNaN(font_scale) && typeof font_scale === "number")
                            {
                                par_font_size = parg.CompiledPr.Pr.TextPr.FontSize;

                                for(r = 0; r < parg.Content.length; ++r)
                                {
                                    item = parg.Content[r];
                                    if(item.Type === para_TextPr)
                                    {
                                ...
                                result_par_text_pr_font_size = (par_font_size*font_scale) >> 0;
                                if(!isNaN(result_par_text_pr_font_size) && typeof result_par_text_pr_font_size === "number")
                                {
                                    b_insert_text_pr = false;
                                    pos = -1;
                                    for(p = 0; p < parg.Content.length; ++p)
                                    {
                                        if(parg.Content[p].Type === para_TextPr)
                                        {
                                            if(!(!isNaN(parg.Content[p].Value.FontSize) && typeof parg.Content[p].Value.FontSize === "number"))
                    ...
                    DCHeight=0; //высота текста
                    for(i = 0, n = this.content.Get_PagesCount();  i<n; ++i)
                    {
                        bounds = this.content.Get_PageBounds(i);
                        DCHeight += bounds.Bottom-bounds.Top;
                    }
    ...
    calculateText2 : function(Theme, slide, Layout, Master, fontRef)
    {
        ...
        if(this.content)
        {
            for(var i = 0; i < this.content.Content.length; ++i)
            {
                this.content.Content[i].Recalc_CompileParaPr();
        ...
        if(this.content2 && this.content2.Content[0] )
        {
            ...
            for(i = 0; i < this.content2.Content.length; ++i)
            {
                ...
                this.content2.Content[i].Recalc_CompileParaPr();
