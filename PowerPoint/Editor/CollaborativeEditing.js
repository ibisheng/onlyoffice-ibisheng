...
function CCollaborativeEditing()
{
    ...
    this.m_aDC          = []; // Массив классов DocumentContent
    ...
    this.Add_Unlock = function(Lock)
    {
        this.m_aNeedUnlock.push( Lock );
    };
    ...
    this.Apply_Changes = function()
    {
        ...
        for ( var Index = 0; Index < UnlockCount; Index++ )
            this.m_aNeedUnlock[Index].Set_Type( locktype_None, false);
        ...
        for ( var Index = 0; Index < UnlockCount2; Index++ )
        {
            ...
            editor.CoAuthoringApi.releaseLocks( Class.Get_Id(), null, false );
        }
    ...
    this.OnEnd_Load_Objects = function()
    {
        ...
        LogicDocument.Internal_Recalculate_After_UndoRedo(RecalculateData);
    };
//-----------------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------------
    ...
    {
        var Count = this.m_aDC.length;
        for ( var Index = 0; Index < Count; Index++ )
        {
            this.m_aDC[Index].Clear_ContentChanges();
        }
    };
    ...
    this.Refresh_DCChanges = function()
    {
        ...
        for ( var Index = 0; Index < Count; Index++ )
        {
            this.m_aDC[Index].Refresh_ContentChanges();
        }
