...
Paragraph.prototype =
{
    ...
    CopyPr_Open : function(OtherParagraph)
    {
        ...
        if ( null != NumPr  )
        {
            ...
            if(this.level)
            {
                OtherParagraph.level = this.level;
            }
            else
            {
                OtherParagraph.level = 0;
            }
            if(this.bullet)
            {
                OtherParagraph.bullet = this.bullet.createDuplicate();
            }
        }
    ...
    {
        if ( para_NewLineRendered != Item.Type && para_PageBreakRendered != Item.Type && para_FlowObjectAnchor != Item.Type && para_InlineBreak != Item.Type )
        {
    ...
    // Добавляем несколько элементов в конецэлемент в содержимое параграфа.
    Internal_Content_Concat : function(Items)
    {
        var StartPos = this.Content.length;
        this.Content = this.Content.concat( Items );
        History.Add( this, { Type : historyitem_Paragraph_AddItem, Pos : this.Internal_Get_ClearPos( StartPos ), EndPos : this.Internal_Get_ClearPos( this.Content.length - 1 ), Items : Items } );
    },
    ...
    Internal_Content_Remove : function (Pos)
    {
        ...
        if ( para_NewLineRendered != Item.Type && para_PageBreakRendered != Item.Type && para_FlowObjectAnchor != Item.Type && para_InlineBreak != Item.Type && para_CollaborativeChangesEnd != Item.Type && para_CollaborativeChangesStart != Item.Type )
        {
            var ClearPos = this.Internal_Get_ClearPos( Pos );
    ...
    Internal_Content_Remove2 : function(Pos, Count)
    {
        ...
        History.Add( this, { Type : historyitem_Paragraph_RemoveItem, Pos : this.Internal_Get_ClearPos( Pos ), EndPos : this.Internal_Get_ClearPos(Pos + Count - 1), Items : LastArray } );
    ...
    Internal_Recalculate_1 : function()
    {
        ...
        CurTextPr.FontSize_S = CurTextPr.FontSize;
        ...
        switch ( CurTextPr.VertAlign )
        {
            ...
            {
                CurTextPr.FontSize = CurTextPr.FontSize_S * vertalign_Koef_Size;
                break;
            }
        }
        ...
        if ( "undefined" == typeof(g_oTextMeasurer) )
            g_oTextMeasurer = Canvas;
        ...
        this.Pages[0].XLimit    = this.XLimit;
        this.Pages[0].YLimit    = this.YLimit;
        this.Pages[0].FirstLine = 0;
        this.Pages[0].Bounds    = { Left : this.X, Top : this.Y, Right : this.XLimit, Bottom : this.Y };
        ...
        var LineStart_Pos    = 0;
        var LineStart_TextPr = Styles_Copy_TextPr( CurTextPr );
        ...
        if ( true === ParaPr.PageBreakBefore )
        {
            ...
            if ( null != Prev )
            {
                ...
                this.Pages[CurPage].YLimit    = PageStart.YLimit;
                this.Pages[CurPage].FirstLine = 0;
                this.Pages[CurPage].Bounds    = { Left : PageStart.X, Top : Y, Right : PageStart.XLimit, Bottom : Y };
            }
        }
        ...
        {
            //TextAscent  = g_oTextMeasurer.GetAscender();
        ...
        {
            Ascent      : 0,//TextAscent,  // Высота над BaseLine
            Descent     : 0,//TextDescent, // Высота после BaseLine
            TextAscent  : 0,//TextAscent,  // Высота текста над BaseLine
            TextDescent : 0,//TextDescent, // Высота текста после BaseLine
        ...
        this.Lines[CurLine].Ranges[0].XEnd   = (RangesCount == 0 ? XLimit : Ranges[0].X0);
        ...
        for ( var Index = 1; Index < Ranges.length + 1; Index++ )
        {
            ...
            this.Lines[CurLine].Ranges[Index].XEnd   = (Index == RangesCount ? XLimit : Ranges[Index].X0);
        }
        ...
        for ( var Pos = LineStart_Pos; Pos < this.Content.length; Pos++ )
        {
            ...
            switch( Item.Type )
            {
                case para_Numbering:
                    ...
                    if(this.NumPr == undefined)
                    {
                        break;
                    }
                    var Numbering = this.Parent.Get_Numbering();
                    if(Numbering.Get_AbstractNum( Item.NumPr.NumId ) == null)//TODO
                    {
                        break;
                    }
                    var NumLvl = Numbering.Get_AbstractNum( Item.NumPr.NumId ).Lvl[Item.NumPr.Lvl];
                    var NumSuff = NumLvl.Suff;
                    var NumJc = NumLvl.Jc;
                    var NumInfo = this.Parent.Internal_GetNumInfo( this.Id, this.NumPr );

                    var NumTextPr = this.Get_CompiledPr().TextPr;
                    Common_CopyObj2( NumTextPr, this.TextPr.Value );
                    Common_CopyObj2( NumTextPr, NumLvl.TextPr );

                    // Здесь измеряется только ширина символов нумерации, без суффикса
                    Item.Measure( g_oTextMeasurer, Numbering, NumInfo, NumTextPr );
                ...
                case para_Text:
                    ...
                    //TextDescent = TextHeight - TextAscent;
                    ...
                    if ( !bWord )
                    {
                        ...
                        var LetterLen = Item.Measure( g_oTextMeasurer ).Width;
                        ...
                        {
                            nWordStartPos = Pos;
                            nWordLen = Item.Measure( g_oTextMeasurer ).Width;
                            bWord = true;

                            if ( "undefined" == typeof(this.Lines[CurLine].Words) )
                                this.Lines[CurLine].Words = 1;
                            else
                        ...
                        if ( X + nSpaceLen + nWordLen + LetterLen > XEnd )
                        {
                            if ( bFirstItemOnLine )
                            {
                                ...
                                if ( 0 == RangesCount )
                                {
                                    ...
                                    // Рассчет метрик строки
                                    if ( LineTextAscent > this.Lines[CurLine].Metrics.TextAscent )
                                        this.Lines[CurLine].Metrics.TextAscent = LineTextAscent;

                                    if ( LineTextDescent > this.Lines[CurLine].Metrics.TextDescent )
                                        this.Lines[CurLine].Metrics.TextDescent = LineTextDescent;

                                    if ( LineAscent > this.Lines[CurLine].Metrics.Ascent )
                                        this.Lines[CurLine].Metrics.Ascent = LineAscent;

                                    if ( LineDescent > this.Lines[CurLine].Metrics.Descent )
                                        this.Lines[CurLine].Metrics.Descent = LineDescent;

                                    this.Lines[CurLine].Metrics.LineGap = this.Internal_Recalculate_1_LineGap( ParaPr, this.Lines[CurLine].Metrics.TextAscent, this.Lines[CurLine].Metrics.TextDescent );
                                    //this.Lines[CurLine].Metrics.LineGap = ( this.Lines[CurLine].Metrics.TextAscent + this.Lines[CurLine].Metrics.TextDescent ) * ( ParaPr.Spacing.Line - 1 );
                                }
                                else if ( RangesCount != CurRange )
                                {
                                    var WordTextPr = this.Internal_CalculateTextPr( nWordStartPos );
                                    CurTextPr = WordTextPr;
                                    CurTextPr.FontSize_S = CurTextPr.FontSize;
                ...
                case para_Space:
                    ...
                    var SpaceLen = Item.Measure( g_oTextMeasurer ).Width;
                    if ( bWord )
                    {
                        // Добавляем длину пробелов до слова
                        X += nSpaceLen;
                ...
                case para_Tab:
                {
                    ...
                    for ( var Index = 0; Index < ParaPr.Tabs.length; Index++ )
                    {
                        ...
                        {
                            NewX = TabPos;
                            Item.TabType = ParaPr.Tabs[Index].Val;
                            break;
                        }
                    }
                ...
                case para_End:
                    ...
                    var EndTextPr = this.Get_CompiledPr().TextPr;
                    Common_CopyObj2( EndTextPr, this.TextPr.Value );
            ...
            if ( bNewLine || ( bNeedNewLine && Pos < this.Content.length - 1 && para_Text == this.Content[Pos + 1].Type ) )
            {
                ...
                if ( true === bEmptyLine )
                {
                    var TextPr_endline = this.Internal_CalculateTextPr( Pos );
                    TextPr_endline.FontSize_S = TextPr_endline.FontSize;
                    switch ( CurTextPr.VertAlign )
                    {
                        case vertalign_Baseline:
                        {
                            TextPr_endline.FontSize = TextPr_endline.FontSize_S;
                            break;
                        }
                        case vertalign_SubScript:
                        case vertalign_SuperScript:
                        {
                            TextPr_endline.FontSize = TextPr_endline.FontSize_S * vertalign_Koef_Size;
                            break;
                        }
                    }

                    if ( Math.abs(TextPr_endline.FontSize - TextPr_endline.FontSize_S) < 0.01 )
                    {
                        TextDescent = Math.abs( g_oTextMeasurer.GetDescender() );
                        TextHeight  = g_oTextMeasurer.GetHeight();
                    }
                    ...
                    {
                        var OldSize = TextPr_endline.FontSize;
                        TextPr_endline.FontSize = TextPr_endline.FontSize_S;
                        g_oTextMeasurer.SetFont( TextPr_endline );
                        TextDescent = Math.abs( g_oTextMeasurer.GetDescender() );
                        TextHeight  = g_oTextMeasurer.GetHeight();
                        TextPr_endline.FontSize = OldSize;
                        g_oTextMeasurer.SetFont( TextPr_endline );
                    }
                    TextAscent = TextHeight - TextDescent;
                ...
                if ( (Top > this.YLimit || Bottom > this.YLimit ) && ( CurLine != this.Pages[CurPage].FirstLine || ( 0 === CurPage && ( null != this.Get_DocumentPrev() || true === this.Parent.Is_TableCellContent() ) ) ) )
                {
                    ...
                    if ( true === ParaPr.KeepLines && null != this.Get_DocumentPrev() && true != this.Parent.Is_TableCellContent() && 0 === CurPage )
                    {
                        ...
                        LineStart_TextPr = Styles_Copy_TextPr( Pr.TextPr );
                        // Для Subscript и superscript добавим дополнительный параметр с размером текста
                        LineStart_TextPr.FontSize_S = LineStart_TextPr.FontSize;

                        switch ( LineStart_TextPr.VertAlign )
                        {
                            case vertalign_Baseline:
                            {
                                LineStart_TextPr.FontSize = LineStart_TextPr.FontSize_S;
                                break;
                            }
                            case vertalign_SubScript:
                            case vertalign_SuperScript:
                            {
                                LineStart_TextPr.FontSize = LineStart_TextPr.FontSize_S * vertalign_Koef_Size;
                                break;
                            }
                        }
                    }
                    ...
                    this.Pages[CurPage].XLimit    = PageStart.XLimit;
                    this.Pages[CurPage].YLimit    = PageStart.YLimit;
                    this.Pages[CurPage].FirstLine = CurLine;
                    this.Pages[CurPage].Bounds    = { Left : PageStart.X, Top : Y, Right : PageStart.XLimit, Bottom : Y };

                    CurTextPr = Styles_Copy_TextPr( LineStart_TextPr );
                    Pos       = LineStart_Pos - 1;
                    g_oTextMeasurer.SetFont( CurTextPr );
                    ...
                    //LineGap    = TextHeight * ( ParaPr.Spacing.Line - 1 );
                    ...
                    for ( var Index = 1; Index < Ranges.length + 1; Index++ )
                    {
                        ...
                        this.Lines[CurLine].Ranges[Index].Words  = 0;
                        this.Lines[CurLine].Ranges[Index].Spaces = 0;
                        this.Lines[CurLine].Ranges[Index].XEnd   = (RangesCount == Index ? XLimit : Ranges[Index].X0);
                    }
                ...
                else
                {
                    ...
                    if ( true === bNewPage )
                    {
                        ...
                        this.Pages[CurPage].XLimit    = PageStart.XLimit;
                        this.Pages[CurPage].YLimit    = PageStart.YLimit;
                        this.Pages[CurPage].FirstLine = CurLine + 1;
                        this.Pages[CurPage].Bounds    = { Left : PageStart.X, Top : Y, Right : PageStart.XLimit, Bottom : Y };
                    }
                    else
                    {
                    ...
                    if ( !bEnd )
                    {
                        ...
                        LineStart_Pos    = Pos + 1;
                        LineStart_TextPr = Styles_Copy_TextPr( CurTextPr );
                    }
        ...
        for ( var ItemNum = 0; ItemNum < this.Content.length; ItemNum++ )
        {
            if ( ItemNum == this.CurPos.ContentPos )
            {
                ...
                if ( true === UpdateTarget )
                {
                    ...
                    var Ascender = Height - Descender;//g_oTextMeasurer.GetAscender();
            ...
            if ( bFindPos && Pos == ItemNum )
            {
                if ( true === ReturnTarget )
                {
                    ...
                    return { X : X, Y : TargetY, Height : Height };
                }
                else
                    return { X : X, Y : Y, PageNum : CurPage + this.Get_StartPage_Absolute() };
            }
    ...
    Internal_CompareBrd : function(Pr1, Pr2)
    {
        ...
        if ( false === Styles_Compare_Brd( Pr1.Brd.Top, Pr2.Brd.Top )   || false === Styles_Compare_Brd( Pr1.Brd.Bottom, Pr2.Brd.Bottom ) ||
             false === Styles_Compare_Brd( Pr1.Brd.Left, Pr2.Brd.Left ) || false === Styles_Compare_Brd( Pr1.Brd.Right, Pr2.Brd.Right )   ||
             false === Styles_Compare_Brd( Pr1.Brd.Between, Pr2.Brd.Between ) )
            return false;
    ...
    Internal_Get_ClearPos : function(Pos)
    {
        ...
        for ( var Index = 0; Index < Math.min(Pos, this.Content.length - 1); Index++ )
        {
            ...
            if ( para_NewLineRendered === ItemType || para_InlineBreak === ItemType || para_PageBreakRendered === ItemType || para_FlowObjectAnchor === ItemType || para_CollaborativeChangesEnd === ItemType || para_CollaborativeChangesStart === ItemType )
                Counter++;
        }
        return Pos - Counter;
    },
    ...
    Internal_Get_ClearContenLength : function()
    {
    ...
    calculateTextTheme : function(theme, slide, layout, master, fontRef)
    {
        ...
        if(this.rPr.unifill && this.rPr.unifill.fill)
        {
            ...
            brush.merge(this.rPr.unifill.fill);
        ...
        for(var i = 0 ; i<this.Content.length; ++i)
        {
            if(this.Content[i].Type == para_TextPr)
            {
                if(this.Content[i].Value.unifill && this.Content[i].Value.unifill.fill)
                {
                    ...
                    brush.calculate(theme, slide, layout, master, RGBA);
                    if(!(brush.fill instanceof  CNoFill))
                    {
    ...
    Recalculate : function()
    {
        ...
        for ( var Pos = 0; Pos < this.Content.length; Pos++ )
        {
            ...
            // Переносим нумерацию в начало параграфа
            else if ( Item.Type == para_Numbering )
            {
                if ( 0 != Pos )
                {
                    this.Internal_Content_Remove( Pos );
                    this.Internal_Content_Add( 0, Item );
                }
            }
            // Удаляем рассчитанные переносы строк
            else if ( Item.Type == para_NewLineRendered || Item.Type == para_InlineBreak || Item.Type == para_PageBreakRendered )
            {
                this.Internal_Content_Remove( Pos );
                Pos--;
            }
            else if ( ( Item.Type == para_CollaborativeChangesEnd || Item.Type == para_CollaborativeChangesStart ) && true === this.DeleteCollaborativeMarks )
            {
                this.Internal_Content_Remove( Pos );
                Pos--;
            }
            // Удаляем подряд идущие TextPr (только если на втором не стоит текущая позиция)
            else if ( Item.Type == para_TextPr && Pos > 0 && this.Content[Pos - 1].Type == para_TextPr && Pos != this.CurPos.ContentPos )
            {
            ...
            else if ( Item.Type === para_NewLine && break_Page === Item.BreakType )
            {
                Item.Flags.NewLine = true;
            }
            else if ( Item.Type === para_Empty && true === Item.Check_Delete() )
            {
                this.Internal_Content_Remove( Pos );
                Pos--;
            }
        ...
        for ( var ItemNum = StartPos; ItemNum < this.Content.length; ItemNum++ )
        {
            ...
            switch( Item.Type )
            {
                case para_Numbering:
                    ...
                    var Numbering = this.Parent.Get_Numbering();
                    var NumInfo   = this.Parent.Internal_GetNumInfo( this.Id, this.NumPr );
                    if(Numbering.Get_AbstractNum( Item.NumPr.NumId ) == undefined)
                    {
                        break;//TODO
                    }
                    var NumLvl    = Numbering.Get_AbstractNum( Item.NumPr.NumId ).Lvl[Item.NumPr.Lvl];
        ...
        for ( var ItemNum = StartPos; ItemNum < this.Content.length; ItemNum++ )
        {
            ...
            switch( Item.Type )
            {
                case para_Numbering:
                    ...
                    var NumTextPr = this.Get_CompiledPr().TextPr; // TODO :


                    // Word не рисует подчеркивание у символа списка, если оно пришло из настроек для
                    // символа параграфа.
                    var TextPr_temp = Styles_Copy_TextPr( this.TextPr.Value );
                    if ( "undefined" != typeof(TextPr_temp) )
                        delete TextPr_temp.Underline;

                    Common_CopyObj2( NumTextPr, TextPr_temp );
                    Common_CopyObj2( NumTextPr, NumLvl.TextPr );
                    if(i != this.Content.length)
                    {
                        Common_CopyObj2( NumTextPr, this.Content[i].Value );
                    }
                    ...
                    {
                        case align_Right:
                            Item.Draw( X - Item.WidthNum, Y, pGraphics, Numbering, NumInfo, NumTextPr );
                            break;

                        case align_Center:
                            Item.Draw( X - Item.WidthNum / 2, Y, pGraphics, Numbering, NumInfo, NumTextPr );
                            break;

                        case align_Left:
                        default:
                            Item.Draw( X, Y, pGraphics, Numbering, NumInfo, NumTextPr );
                            break;
                    }
    ...
    Internal_RemoveBackward : function(bOnlyText)
    {
        ...
        else
        {
            ...
            else if ( null != this.Numbering_Get() )
            {
                // Если у нас параграф с нумерацей, тогда удаляем нумерацию
    ...
    Internal_CalculateTextPr : function (LetterPos, StartPr)
    {
        ...
        var TextPr = Styles_Copy_TextPr( Pr.TextPr );
        ...
        if ( true === Pos.Found )
        {
            ...
            if(CurTextPr.unifill && CurTextPr.unifill.fill == undefined)
            {
                delete CurTextPr.unifill;
            }
    ...
    IncDec_FontSize : function(bIncrease)
    {
        ...
        if ( true === this.ApplyToAll )
        {
            ...
            for ( var Index = 1; Index < this.Content.length; Index++ )
            {
                ...
                if ( para_TextPr === Item.Type )
                {
                    if ( "undefined" != typeof(Item.Value.FontSize) && null != Item.Value.FontSize )
            ...
            // Выставляем настройки для символа параграфа
            if ( "undefined" != typeof(this.TextPr.Value.FontSize) && null != this.TextPr.Value.FontSize )
        ...
        if ( true === this.Selection.Use )
        {
            ...
            if ( "undefined" != typeof(TextPr_start.FontSize) && null != TextPr_start.FontSize )
        ...
        else if ( para_PageNum === CurType || para_Drawing === CurType || para_Tab == CurType || para_Space == CurType || para_NewLine == CurType || !oStart.Found || para_NewLine == oEnd.Type || para_Space == oEnd.Type || para_NewLine == oStart.Type || para_Space == oStart.Type || para_Tab == oEnd.Type || para_Tab == oStart.Type || para_Drawing == oEnd.Type || para_Drawing == oStart.Type || para_PageNum == oEnd.Type || para_PageNum == oStart.Type )
        {
            ...
            var TextPr_new = Styles_Copy_TextPr( TextPr_old );

            if ( "undefined" != typeof(TextPr_new.FontSize) && null != TextPr_new.FontSize )
    ...
    IncDec_Indent : function(bIncrease)
    {
        ...
        else
        {
            ...
            this.Set_Ind( { Left : LeftMargin_new } );
        }
    },
    ...
    Cursor_MoveEndOfLine : function(AddToSelect)
    {
        ...
        if ( true === this.Selection.Use )
        {
            ...
            {
                this.CurPos.ContentPos = ( this.Selection.EndPos >= this.Selection.StartPos ? this.Selection.EndPos : this.Selection.StartPos );
                this.Selection_Remove();
            }
        }
    ...
    Get_Styles : function()
    {
        var lvl = this.NumPr ? (this.NumPr.Lvl != null ? this.NumPr.Lvl : 0 ) : 0;
        var Styles = this.Parent.Parent.Get_Styles(lvl);
    ...
    Internal_FindWordStart : function(Pos, Pos_min)
    {
        ...
        else
        {
            ...
            while ( TempPos > Pos_min )
            {
                ...
                if ( !( para_TextPr === TempType || ( para_Text === TempType && true != Item.Is_NBSP() && ( ( true === bPunctuation && true === Item.Is_Punctuation() ) || ( false === bPunctuation && false === Item.Is_Punctuation() ) ) ) ) )
                    break;
                else
                    LetterPos = TempPos;
            }
    ...
    Internal_FindWordEnd : function(Pos, Pos_max)
    {
        ...
        else
        {
            ...
            while ( TempPos < Pos_max )
            {
                ...
                var Item = this.Content[TempPos]
                var TempType = Item.Type;
                if ( !( para_TextPr === TempType || para_Space === this.Content[LetterPos].Type || ( para_Text === this.Content[LetterPos].Type && true === this.Content[LetterPos].Is_NBSP() ) ) )
                    break;
                else
                    LetterPos = TempPos;
            }
            ...
            for ( var Pos = 0; Pos < this.Content.length; Pos++ )
            {
                if ( this.Content[Pos].Type == para_TextPr )
                {
                    for ( var Item in TextPr )
                        this.Content[Pos].Set_Prop( Item, TextPr[Item] );
                }
            }

            // Выставляем настройки для символа параграфа
            for ( var Item in TextPr )
                this.TextPr.Set_Prop( Item, TextPr[Item] );
        ...
        for ( var ItemNum = 0; ItemNum < this.Content.length; ItemNum++ )
        {
            ...
            if ( CurLine == CurLine2 )
            {
                ...
                if ( bCheck )
                {
                    ...
                    if ( true === bNumbering )
                    {
                        var NumJc = this.Parent.Get_Numbering().Get_AbstractNum( Item.NumPr.NumId ).Lvl[Item.NumPr.Lvl].Jc;
    ...
    Hyperlink_Add : function(HyperProps)
    {
        ...
        else if ( null != HyperProps.Text && "" != HyperProps.Text ) // добавлять ссылку, без селекта и с пустым текстом нельзя
        {
            ...
            TextPr_hyper.Color     = { r : 0, g : 0, b : 255 };
    ...
    Selection_SetEnd : function(X,Y,PageNum, MouseEvent, bTableBorder)
    {
        ...
        if ( true === this.Selection.Use )
        {
            this.Selection_CalculateTextPr();
        }
    },
        ...
        if ( true === this.Selection.Use )
        {
            switch ( this.Selection.Flag )
            {
                ...
                case  selectionflag_Numbering:
                {
                    ...
                    var NumJc = this.Parent.Get_Numbering().Get_AbstractNum( ParaNum.NumPr.NumId ).Lvl[ParaNum.NumPr.Lvl].Jc;
                    switch ( NumJc )
                    {
                        case align_Center:
        ...
        if ( true === this.Selection.Use || true === this.ApplyToAll )
        {
            ...
            for ( var Pos = StartPos + 1; Pos < EndPos; Pos++ )
            {
                ...
                if ( para_TextPr == Item.Type && Pos < this.Content.length - 1 && para_TextPr != this.Content[Pos + 1].Type )
                {
                    ...
                    TextPr_vis = Styles_Compare_TextPr( TextPr_vis, TextPr_cur );
                }
            }
    ...
    Numbering_Add : function(NumId, Lvl)
    {
        ...
        var NumPr_old = null;
        if ( "undefined" != typeof(this.NumPr) )
            NumPr_old = this.NumPr;
        ...
        History.Add( this, { Type : historyitem_Paragraph_Numbering, Old : NumPr_old, New : this.NumPr } );

        // Добавляем также специальные знаки в начало параграфа
        this.Internal_Content_Add( 0, new ParaNumbering(this.NumPr) );

        if ( "undefined" != typeof(this.Pr.Ind) )
        {
            History.Add( this, { Type : historyitem_Paragraph_Ind_First, Old : ( "undefined" != typeof(this.Pr.Ind.FirstLine) ? this.Pr.Ind.FirstLine : null ), New : null } );
            History.Add( this, { Type : historyitem_Paragraph_Ind_Left,  Old : ( "undefined" != typeof(this.Pr.Ind.Left)      ? this.Pr.Ind.Left      : null ), New : null } );

            // При добавлении списка в параграф, удаляем все собственные сдвиги
            delete this.Pr.Ind.FirstLine;
            delete this.Pr.Ind.Left;
        }
    ...
    {
        // Ищем элемент с типом para_Numbering и удаляем его
        for ( var Index = 0; Index < this.Content.length; Index++ )
        {
            if ( this.Content[Index].Type == para_Numbering )
            {
                this.Internal_Content_Remove( Index );
                break;
            }
    ...
    {
        History.Add( this, { Type : historyitem_Paragraph_Numbering, Old : (  "undefined" != typeof(this.NumPr) ? this.NumPr : null ), New : null } );
    ...
    {
        var bLvl = ("undefined" === typeof(Lvl) ? false : true);

        var NumPr = null;
        if ( null != ( NumPr = this.Numbering_Get() ) && NumId === NumPr.NumId && ( false === bLvl || Lvl === NumPr.Lvl ) )
            return true;
    ...
    Get_CompiledPr : function()
    {
        ...
        if ( null != PrevEl && type_Paragraph === PrevEl.GetType() )
        {
            ...
            {
                if ( true === Cur_BeforeAuto && PrevStyle === StyleId && null != Prev_NumPr && null != NumPr && Prev_NumPr.NumId === NumPr.NumId )
                    Pr.ParaPr.Spacing.Before = 0;
                else
                {
        ...
        if ( null != NextEl )
        {
            if ( type_Paragraph === NextEl.GetType() )
            {
                ...
                {
                    if ( true === Cur_AfterAuto && NextStyle === StyleId && null != Next_NumPr && null != NumPr && Next_NumPr.NumId === NumPr.NumId )
    ...
    Get_CompiledPr2 : function(bCopy)
    {
        ...
        if ( false === bCopy )
            ...
            return Styles_Copy_Pr( this.CompiledPr.Pr ); // Отдаем копию объекта, чтобы никто не поменял извне настройки стиля
    },
    ...
    Internal_CompileParaPr : function()
    {
       /* var NumPr   = this.Numbering_Get();
        ...
        var NumPr   = this.Numbering_Get();
        if(this.NumPr)
            var level = this.NumPr.Lvl;
        else
            level = 0;
        var Styles  = this.Get_Styles(level);
        var StyleId;
        StyleId = Styles.Style.length-1;

        // Считываем свойства для текущего стиля
        var Pr = Styles.Get_Pr( StyleId, styles_Paragraph );
        if (false && this.NumPr )
        {
            var Numbering = this.Parent.Get_Numbering();
            if ( null != Numbering )
                Common_CopyObj2( Pr.ParaPr, Numbering.Get_ParaPr( NumPr.NumId, NumPr.Lvl ) );
        }

        // Копируем прямые настройки параграфа.
        Common_CopyObj2( Pr.ParaPr, this.Pr );
        Common_CopyObj2( Pr.TextPr, this.rPr );
        if(Pr.TextPr.unifill && Pr.TextPr.unifill.fill)
        {
            var shape = this.Parent.Parent;
            if(shape instanceof CShape)//TODO :
            {
                ...
                var brush = null;
                var RGBA = {R:0, G:0, B:0, A:255};
                if (theme && shape.style!=null && shape.fontRef!=null)
                {
                    brush = theme.getFillStyle(shape.fontRef.idx);
                    shape.fontRef.Color.Calculate(theme, slide, layout, master);
                    RGBA = shape.fontRef.Color.RGBA;

                    if (shape.fontRef.Color.color != null)
                    {
                        if (brush.fill != null && brush.fill.type == FILL_TYPE_SOLID)
                        {
                            brush.fill.color = shape.fontRef.Color.createDuplicate();
                        }
                    }
                }
                else
    ...
    Get_Paragraph_ParaPr_Copy : function()
    {
        ...
        if ( "undefined" != typeof(this.Pr.Ind) )
        {
            ParaPr.Ind =
            {
                Left      : "undefined" != typeof(this.Pr.Ind.Left)      ? this.Pr.Ind.Left      : null,
                Right     : "undefined" != typeof(this.Pr.Ind.Right)     ? this.Pr.Ind.Right     : null,
                FirstLine : "undefined" != typeof(this.Pr.Ind.FirstLine) ? this.Pr.Ind.FirstLine : null
            };
        }
        else
        {
        ...
        // Jc
        if ( "undefined" != typeof(this.Pr.Jc) )
            ParaPr.Jc = this.Pr.Jc;
        else
            ParaPr.Jc = null;
        ...
        if ( "undefined" != typeof(this.Pr.Spacing) )
        {
            ParaPr.Spacing =
            {
                After    : "undefined" != typeof(this.Pr.Spacing.After)    ? this.Pr.Spacing.After    : null,
                Before   : "undefined" != typeof(this.Pr.Spacing.Before)   ? this.Pr.Spacing.Before   : null,
                Line     : "undefined" != typeof(this.Pr.Spacing.Line)     ? this.Pr.Spacing.Line     : null,
                LineRule : "undefined" != typeof(this.Pr.Spacing.LineRule) ? this.Pr.Spacing.LineRule : null
            };
        }
        else
        {
        ...
        // PageBreakBefore
        if ( "undefined" != typeof(this.Pr.PageBreakBefore) )
            ParaPr.PageBreakBefore = this.Pr.PageBreakBefore;
        else
            ParaPr.PageBreakBefore = null;

        // KeepLines
        if ( "undefined" != typeof(this.Pr.KeepLines) )
            ParaPr.KeepLines = this.Pr.KeepLines;
        else
            ParaPr.KeepLines = null;

        // ContextualSpacing
        if ( "undefined" != typeof(this.Pr.ContextualSpacing) )
            ParaPr.ContextualSpacing = this.Pr.ContextualSpacing;
        else
            ParaPr.ContextualSpacing = null;

        // Shd
        if ( "undefined" != typeof(this.Pr.Shd) )
            ParaPr.Shd = Common_CopyObj( this.Pr.Shd );

        // StyleId
        if ( "undefined" != typeof(this.ParaStyle) )
            ParaPr.StyleId = this.ParaStyle;
        ...
        if ( "undefined" != typeof(this.Pr.Brd) )
        {
            ...
            if ( "undefined" != typeof(this.Pr.Brd.Bottom) )
                ParaPr.Brd.Bottom = Styles_Copy_Brd( this.Pr.Brd.Bottom );

            if ( "undefined" != typeof(this.Pr.Brd.Left) )
                ParaPr.Brd.Left = Styles_Copy_Brd( this.Pr.Brd.Left );

            if ( "undefined" != typeof(this.Pr.Brd.Right) )
                ParaPr.Brd.Right = Styles_Copy_Brd( this.Pr.Brd.Right );

            if ( "undefined" != typeof(this.Pr.Brd.Top) )
                ParaPr.Brd.Top = Styles_Copy_Brd( this.Pr.Brd.Top );
        }
    ...
    Paragraph_Format_Paste : function(TextPr, ParaPr, ApplyPara)
    {
        ...
        {
            // Ind
            if ( "undefined" != typeof(ParaPr.Ind) )
                this.Set_Ind( ParaPr.Ind );

            // Jc
            if ( "undefined" != typeof(ParaPr.Jc) )
                this.Set_Align( ParaPr.Jc );

            // Spacing
            if ( "undefined" != typeof(ParaPr.Spacing) )
                this.Set_Spacing( ParaPr.Spacing );

            // PageBreakBefore
            if ( "undefined" != typeof(ParaPr.PageBreakBefore) )
                this.Set_PageBreakBefore( ParaPr.PageBreakBefore );

            // KeepLines
            if ( "undefined" != typeof(ParaPr.KeepLines) )
                this.Set_KeepLines( ParaPr.KeepLines );

            // ContextualSpacing
            if ( "undefined" != typeof(ParaPr.ContextualSpacing) )
                this.Set_ContextualSpacing( ParaPr.ContextualSpacing );

            // Shd
            if ( "undefined" != typeof(ParaPr.Shd) )
                this.Set_Shd( ParaPr.Shd );

            // StyleId
            if ( "undefined" != typeof(ParaPr.StyleId) )
                this.Style_Add( ParaPr.StyleId, true );
            else
                this.Style_Remove();

            // NumPr
            if ( "undefined" != typeof(ParaPr.NumPr) )
                this.Numbering_Add( ParaPr.NumPr.NumId, ParaPr.NumPr.Lvl );
            else
                this.Numbering_Remove();

            // Brd
            if ( "undefined" != typeof(ParaPr.Brd) )
    ...
    Style_Add : function(Id, bDoNotDeleteProps)
    {
        ...
        {
            History.Add( this, { Type : historyitem_Paragraph_Style, Old : Id_old, New : Id } );
            this.ParaStyle = Id;
        }
        ...
        {
            // При изменении стиля удалются все прямые настройки параграфа
            if ( "undefined" != typeof(this.Pr.ContextualSpacing) )
            {
                History.Add( this, { Type : historyitem_Paragraph_ContextualSpacing, Old : this.Pr.ContextualSpacing, New : null } );
                delete this.Pr.ContextualSpacing;
            }

            if ( "undefined" != typeof(this.Pr.Ind) )
            {
                if ( "undefined" != typeof(this.Pr.Ind.FirstLine) )
                    History.Add( this, { Type : historyitem_Paragraph_Ind_First, Old : this.Pr.Ind.FirstLine, New : null } );

                if ( "undefined" != typeof(this.Pr.Ind.Left) )
                    History.Add( this, { Type : historyitem_Paragraph_Ind_Left,  Old : this.Pr.Ind.Left,      New : null } );

                if ( "undefined" != typeof(this.Pr.Ind.Right) )
                    History.Add( this, { Type : historyitem_Paragraph_Ind_Right, Old : this.Pr.Ind.Right,     New : null } );
            ...
            if ( "undefined" != typeof(this.Pr.Jc) )
            {
                History.Add( this, { Type : historyitem_Paragraph_Align, Old : this.Pr.Jc, New : null } );
                delete this.Pr.Jc;
            }

            if ( "undefined" != typeof(this.Pr.KeepLines) )
            {
                History.Add( this, { Type : historyitem_Paragraph_KeepLines, Old : this.Pr.KeepLines, New : null } );
                delete this.Pr.KeepLines;
            }

            if ( "undefined" != typeof(this.Pr.KeepNext) )
            {
                History.Add( this, { Type : historyitem_Paragraph_KeepNext, Old : this.Pr.KeepNext, New : null } );
                delete this.Pr.KeepNext;
            }

            if ( "undefined" != typeof(this.Pr.PageBreakBefore) )
            {
                History.Add( this, { Type : historyitem_Paragraph_PageBreakBefore, Old : this.Pr.PageBreakBefore, New : null } );
                delete this.Pr.PageBreakBefore;
            }

            if ( "undefined" != typeof(this.Pr.Spacing) )
            {
                if ( "undefined" != typeof(this.Pr.Spacing.Line) )
                    History.Add( this, { Type : historyitem_Paragraph_Spacing_Line,     Old : this.Pr.Spacing.Line,     New : null } );

                if ( "undfeined" != typeof(this.Pr.Spacing.LineRule) )
                    History.Add( this, { Type : historyitem_Paragraph_Spacing_LineRule, Old : this.Pr.Spacing.LineRule, New : null } );

                if ( "undefined" != typeof(this.Pr.Spacing.Before) )
                    History.Add( this, { Type : historyitem_Paragraph_Spacing_Before,   Old : this.Pr.Spacing.Before,   New : null } );

                if ( "undefined" != typeof(this.Pr.Spacing.After) )
                    History.Add( this, { Type : historyitem_Paragraph_Spacing_After,    Old : this.Pr.Spacing.After,    New : null } );

                delete this.Pr.Spacing;
            }

            if ( "undefined" != typeof(this.Pr.Shd) )
            {
                if ( "undefined" != typeof(this.Pr.Shd.Value) )
                    History.Add( this, { Type : historyitem_Paragraph_Shd_Value, Old : this.Pr.Shd.Value, New : null } );

                if ( "undefined" != typeof(this.Pr.Shd.Color) )
                    History.Add( this, { Type : historyitem_Paragraph_Shd_Color, Old : this.Pr.Shd.Color, New : null } );

                delete this.Pr.Shd;
            }

            if ( "undefined" != typeof(this.Pr.WidowControl) )
            {
                History.Add( this, { Type : historyitem_Paragraph_WidowControl, Old : this.Pr.WidowControl, New : null } );
                delete this.Pr.WidowControl;
            }

            if ( "undefined" != typeof(this.Pr.Tabs) )
            {
                History.Add( this, { Type : historyitem_Paragraph_Tabs, Old : this.Pr.Tabs, New : null } );
                delete this.Pr.Tabs;
            }

            if ( "undefined" != typeof(this.Pr.Brd) )
            {
                if ( "undefined" != typeof(this.Pr.Brd.Between) && null != this.Pr.Brd.Between )
                {
                    History.Add( this, { Type : historyitem_Paragraph_Borders_Between, New : null, Old : this.Pr.Brd.Between } );
                    delete this.Pr.Brd.Between;
                }

                if ( "undefined" != typeof(this.Pr.Brd.Bottom) && null != this.Pr.Brd.Bottom )
                {
                    History.Add( this, { Type : historyitem_Paragraph_Borders_Bottom, New : null, Old : this.Pr.Brd.Bottom } );
                    delete this.Pr.Brd.Bottom;
                }

                if ( "undefined" != typeof(this.Pr.Brd.Left) && null != this.Pr.Brd.Left )
                {
                    History.Add( this, { Type : historyitem_Paragraph_Borders_Left, New : null, Old : this.Pr.Brd.Left } );
                    delete this.Pr.Brd.Left;
                }

                if ( "undefined" != typeof(this.Pr.Brd.Right) && null != this.Pr.Brd.Right )
                {
                    History.Add( this, { Type : historyitem_Paragraph_Borders_Right, New : null, Old : this.Pr.Brd.Right } );
                    delete this.Pr.Brd.Right;
                }

                if ( "undefined" != typeof(this.Pr.Brd.Top) && null != this.Pr.Brd.Top )
                {
                    History.Add( this, { Type : historyitem_Paragraph_Borders_Top, New : null, Old : this.Pr.Brd.Top } );
                    delete this.Pr.Brd.Top;
                }
            }
    ...
    Style_Remove : function()
    {
        if ( "undefined" != typeof(this.ParaStyle) )
        {
            History.Add( this, { Type : historyitem_Paragraph_Style, Old : this.ParaStyle, New : null } );
            delete this.ParaStyle;
        }
    ...
    Clear_Formatting : function()
    {
        ...
        if ( "undefined" != typeof(this.Pr.Brd) )
        {
            History.Add( this, { Type : historyitem_Paragraph_Borders_Top,     Old : this.Pr.Brd.Top,     New : null } );
            History.Add( this, { Type : historyitem_Paragraph_Borders_Bottom,  Old : this.Pr.Brd.Bottom,  New : null } );
            History.Add( this, { Type : historyitem_Paragraph_Borders_Left,    Old : this.Pr.Brd.Left,    New : null } );
            History.Add( this, { Type : historyitem_Paragraph_Borders_Right,   Old : this.Pr.Brd.Right,   New : null } );
            History.Add( this, { Type : historyitem_Paragraph_Borders_Between, Old : this.Pr.Brd.Between, New : null } );
            delete this.Pr.Brd;
        }
    ...
    Set_Ind : function(Ind)
    {
        ...
        {
            History.Add( this, { Type : historyitem_Paragraph_Ind_First, New : Ind.FirstLine, Old : ( "undefined" != typeof(this.Pr.Ind.FirstLine) ? this.Pr.Ind.FirstLine : null ) } );
        ...
        {
            History.Add( this, { Type : historyitem_Paragraph_Ind_Left, New : Ind.Left, Old : ( "undefined" != typeof(this.Pr.Ind.Left) ? this.Pr.Ind.Left : null ) } );
        ...
        {
            History.Add( this, { Type : historyitem_Paragraph_Ind_Right, New : Ind.Right, Old : ( "undefined" != typeof(this.Pr.Ind.Right) ? this.Pr.Ind.Right : null ) } );

            if ( null != Ind.Right )
                this.Pr.Ind.Right = Ind.Right;
            else
                delete this.Pr.Ind.Right;
        }
    ...
    Set_Spacing : function(Spacing)
    {
        ...
        {
            History.Add( this, { Type : historyitem_Paragraph_Spacing_Line, New : Spacing.Line, Old : ( "undefined" != typeof(this.Pr.Spacing.Line) ? this.Pr.Spacing.Line : null ) } );

            if ( null != Spacing.Line )
                this.Pr.Spacing.Line = Spacing.Line;
            else
                delete this.Pr.Spacing.Line;
        }

        if ( "undefined" != typeof(Spacing.LineRule) )
        {
            History.Add( this, { Type : historyitem_Paragraph_Spacing_LineRule, New : Spacing.LineRule, Old : ( "undefined" != typeof(this.Pr.Spacing.LineRule) ? this.Pr.Spacing.LineRule : null ) } );
            this.Pr.Spacing.LineRule = Spacing.LineRule;

            if ( null != Spacing.LineRule )
                this.Pr.Spacing.LineRule = Spacing.LineRule;
            else
                delete this.Pr.Spacing.LineRule;
        }

        if ( "undefined" != typeof(Spacing.Before) )
        {
            History.Add( this, { Type : historyitem_Paragraph_Spacing_Before, New : Spacing.Before, Old : ( "undefined" != typeof(this.Pr.Spacing.Before) ? this.Pr.Spacing.Before : null ) } );

            if ( null != Spacing.Before )
                this.Pr.Spacing.Before = Spacing.Before;
            else
                delete this.Pr.Spacing.Before;
        }

        if ( "undefined" != typeof(Spacing.After) )
        {
            History.Add( this, { Type : historyitem_Paragraph_Spacing_After, New : Spacing.After, Old : ( "undefined" != typeof(this.Pr.Spacing.After) ? this.Pr.Spacing.After : null ) } );

            if ( null != Spacing.After )
                this.Pr.Spacing.After = Spacing.After;
            else
                delete this.Pr.Spacing.After;
        }

        if ( "undefined" != typeof(Spacing.AfterAutoSpacing) )
        {
            History.Add( this, { Type : historyitem_Paragraph_Spacing_AfterAutoSpacing, New : Spacing.AfterAutoSpacing, Old : ( "undefined" != typeof(this.Pr.Spacing.AfterAutoSpacing) ? this.Pr.Spacing.AfterAutoSpacing : null ) } );
        ...
        if ( "undefined" != typeof(Spacing.BeforeAutoSpacing) )
        {
            History.Add( this, { Type : historyitem_Paragraph_Spacing_BeforeAutoSpacing, New : Spacing.BeforeAutoSpacing, Old : ( "undefined" != typeof(this.Pr.Spacing.BeforeAutoSpacing) ? this.Pr.Spacing.BeforeAutoSpacing : null ) } );
    ...
    {
        History.Add( this, { Type : historyitem_Paragraph_Align, New : Align, Old : ( "undefined" != typeof(this.Pr.Jc) ? this.Pr.Jc : null ) } );
    ...
    Set_Shd : function(Shd)
    {
        ...
        {
            History.Add( this, { Type : historyitem_Paragraph_Shd_Value, New : Shd.Value, Old : ( "undefined" != typeof(this.Pr.Shd.Value) ? this.Pr.Shd.Value : null ) } );

            if ( null != Shd.Value )
                this.Pr.Shd.Value = Shd.Value;
            else
                delete this.Pr.Shd.Value;
        }

        if ( "undefined" != typeof(Shd.Color) )
        {
            History.Add( this, { Type : historyitem_Paragraph_Shd_Color, New : Shd.Color, Old : ( "undefined" != typeof(this.Pr.Shd.Color) ? this.Pr.Shd.Color : null ) } );

            if ( null != Shd.Color )
                this.Pr.Shd.Color = Shd.Color;
            else
                delete this.Pr.Shd.Color;
        }
    ...
    {
        History.Add( this, { Type : historyitem_Paragraph_Tabs, New : Tabs, Old : ( "undefined" != typeof(this.Pr.Tabs) ? this.Pr.Tabs : null ) } );
        this.Pr.Tabs = Tabs;
    ...
    {
        History.Add( this, { Type : historyitem_Paragraph_ContextualSpacing, New : Value, Old : ( "undefined" != typeof(this.Pr.ContextualSpacing) ? this.Pr.ContextualSpacing : null ) } );
    ...
    {
        History.Add( this, { Type : historyitem_Paragraph_PageBreakBefore, New : Value, Old : ( "undefined" != typeof(this.Pr.PageBreakBefore) ? this.Pr.PageBreakBefore : null ) } );
    ...
    {
        History.Add( this, { Type : historyitem_Paragraph_KeepLines, New : Value, Old : ( "undefined" != typeof(this.Pr.KeepLines) ? this.Pr.KeepLines : null ) } );
    ...
    {
        if ( null === Borders || "undefined" === typeof(Borders) )
            return;
        ...
        {
            var NewBorder = null;
            if ( null != Borders.Between.Value /*&& border_Single === Borders.Between.Value*/ )
                NewBorder =
                {
                    Color : ( null != Borders.Between.Color ? { r : Borders.Between.Color.r, g : Borders.Between.Color.g, b : Borders.Between.Color.b } : { r : OldBrds.Between.Color.r, g : OldBrds.Between.Color.g, b : OldBrds.Between.Color.b } ),
                    Space : ( null != Borders.Between.Space ? Borders.Between.Space : OldBrds.Between.Space ),
                    Size  : ( null != Borders.Between.Size  ? Borders.Between.Size  : OldBrds.Between.Size  ),
                    Value : ( null != Borders.Between.Value ? Borders.Between.Value : OldBrds.Between.Value )
                };

            var OldBorder = null;
            if ( "undefined" != typeof(this.Pr.Brd.Between) )
                OldBorder = this.Pr.Brd.Between;

            History.Add( this, { Type : historyitem_Paragraph_Borders_Between, New : NewBorder, Old : OldBorder } );
        ...
        if ( false === Styles_CheckNullBorder(Borders.Top) )
        {
            var NewBorder = null;
            if ( null != Borders.Top.Value /*&& border_Single === Borders.Top.Value*/ )
                NewBorder =
                {
                    Color : ( null != Borders.Top.Color ? { r : Borders.Top.Color.r, g : Borders.Top.Color.g, b : Borders.Top.Color.b } : { r : OldBrds.Top.Color.r, g : OldBrds.Top.Color.g, b : OldBrds.Top.Color.b } ),
                    Space : ( null != Borders.Top.Space ? Borders.Top.Space : OldBrds.Top.Space ),
                    Size  : ( null != Borders.Top.Size  ? Borders.Top.Size  : OldBrds.Top.Size  ),
                    Value : ( null != Borders.Top.Value ? Borders.Top.Value : OldBrds.Top.Value )
                };

            var OldBorder = null;
            if ( "undefined" != typeof(this.Pr.Brd.Top) )
                OldBorder = this.Pr.Brd.Top;

            History.Add( this, { Type : historyitem_Paragraph_Borders_Top, New : NewBorder, Old : OldBorder } );
        ...
        if ( false === Styles_CheckNullBorder(Borders.Right) )
        {
            var NewBorder = null;
            if ( null != Borders.Right.Value /*&& border_Single === Borders.Right.Value*/ )
                NewBorder =
                {
                    Color : ( null != Borders.Right.Color ? { r : Borders.Right.Color.r, g : Borders.Right.Color.g, b : Borders.Right.Color.b } : { r : OldBrds.Right.Color.r, g : OldBrds.Right.Color.g, b : OldBrds.Right.Color.b } ),
                    Space : ( null != Borders.Right.Space ? Borders.Right.Space : OldBrds.Right.Space ),
                    Size  : ( null != Borders.Right.Size  ? Borders.Right.Size  : OldBrds.Right.Size  ),
                    Value : ( null != Borders.Right.Value ? Borders.Right.Value : OldBrds.Right.Value )
                };

            var OldBorder = null;
            if ( "undefined" != typeof(this.Pr.Brd.Right) )
                OldBorder = this.Pr.Brd.Right;

            History.Add( this, { Type : historyitem_Paragraph_Borders_Right, New : NewBorder, Old : OldBorder } );
        ...
        if ( false === Styles_CheckNullBorder(Borders.Bottom) )
        {
            var NewBorder = null;
            if ( null != Borders.Bottom.Value /*&& border_Single === Borders.Bottom.Value*/ )
                NewBorder =
                {
                    Color : ( null != Borders.Bottom.Color ? { r : Borders.Bottom.Color.r, g : Borders.Bottom.Color.g, b : Borders.Bottom.Color.b } : { r : OldBrds.Bottom.Color.r, g : OldBrds.Bottom.Color.g, b : OldBrds.Bottom.Color.b } ),
                    Space : ( null != Borders.Bottom.Space ? Borders.Bottom.Space : OldBrds.Bottom.Space ),
                    Size  : ( null != Borders.Bottom.Size  ? Borders.Bottom.Size  : OldBrds.Bottom.Size  ),
                    Value : ( null != Borders.Bottom.Value ? Borders.Bottom.Value : OldBrds.Bottom.Value )
                };

            var OldBorder = null;
            if ( "undefined" != typeof(this.Pr.Brd.Bottom) )
                OldBorder = this.Pr.Brd.Bottom;

            History.Add( this, { Type : historyitem_Paragraph_Borders_Bottom, New : NewBorder, Old : OldBorder } );
        ...
        if ( false === Styles_CheckNullBorder(Borders.Left) )
        {
            var NewBorder = null;
            if ( null != Borders.Left.Value /*&& border_Single === Borders.Left.Value*/ )
                NewBorder =
                {
                    Color : ( null != Borders.Left.Color ? { r : Borders.Left.Color.r, g : Borders.Left.Color.g, b : Borders.Left.Color.b } : { r : OldBrds.Left.Color.r, g : OldBrds.Left.Color.g, b : OldBrds.Left.Color.b } ),
                    Space : ( null != Borders.Left.Space ? Borders.Left.Space : OldBrds.Left.Space ),
                    Size  : ( null != Borders.Left.Size  ? Borders.Left.Size  : OldBrds.Left.Size  ),
                    Value : ( null != Borders.Left.Value ? Borders.Left.Value : OldBrds.Left.Value )
                };

            var OldBorder = null;
            if ( "undefined" != typeof(this.Pr.Brd.Left) )
                OldBorder = this.Pr.Brd.Left;

            History.Add( this, { Type : historyitem_Paragraph_Borders_Left, New : NewBorder, Old : OldBorder } );
    ...
    DocumentSearch : function(Str, ElementType)
    {
        ...
        {
            var Item = this.Content[Pos];
            if ( para_Numbering === Item.Type || para_TextPr === Item.Type || para_NewLineRendered === Item.Type || para_PageBreakRendered === Item.Type )
                continue;
    ...
    DocumentStatistics : function(Stats)
    {
        ...
        for ( var Index = 0; Index < this.Content.length; Index++ )
        {
            ...
            if ( para_Text === Item.Type && false === Item.Is_NBSP() )
            {
                if ( false === bWord )
                    bNewWord = true;
    ...
    Update_CursorType : function(X, Y, PageIndex)
    {
        ...
        var Coords = this.DrawingDocument.ConvertCoordsToCursorWR( X, Y, this.Get_StartPage_Absolute() + ( PageIndex - this.PageNum ) );
    ...
    Document_CreateFontMap : function(FontMap)
    {
        ...
        var CurTextPr =
        {
            FontSize : this.CompiledPr.Pr.TextPr.FontSize,
            FontFamily :
            {
                Index : this.CompiledPr.Pr.TextPr.FontFamily.Index,
                Name  : this.CompiledPr.Pr.TextPr.FontFamily.Name
            },
            Bold      : this.CompiledPr.Pr.TextPr.Bold,
            Italic    : this.CompiledPr.Pr.TextPr.Italic,
            VertAlign : this.CompiledPr.Pr.TextPr.VertAlign
        };
        ...
        // Специально для знака параграфа
        Common_CopyObj2( CurTextPr, this.TextPr.Value );
        ...
        for ( var Index = 0; Index < this.Content.length; Index++ )
        {
            ...
            if ( para_TextPr === Item.Type )
            {
                CurTextPr = this.Internal_CalculateTextPr( Index );
    ...
    PreDelete : function()
    {
        ...
        // iline объектах в родительском классе, используемых в данном параграфе.
...
//-----------------------------------------------------------------------------------
    ...
    Split : function(NewParagraph, Pos)
    {
        ...
        for(var i = Pos-1; i > -1; --i )
        {
            if(this.Content[i].Type == para_TextPr)
            {
                NewParagraph.Internal_Content_Add( 0, this.Content[i].createDuplicate() );
            }
        }
       // NewParagraph.Internal_Content_Add( 0, new ParaTextPr( TextPr ) );

        NewParagraph.TextPr.Value = Styles_Copy_TextPr( this.TextPr.Value );
...
//-----------------------------------------------------------------------------------
    Undo : function(Data)
    {
        ...
        switch ( Type )
        {
            ...
            case historyitem_Paragraph_Ind_First:
            {
                ...
                else if ( "undefined" != typeof(this.Pr.Ind) )
                    delete this.Pr.Ind.FirstLine;
            ...
            case historyitem_Paragraph_Spacing_Line:
            {
                ...
                else if ( "undefined" != typeof(this.Pr.Spacing) )
                    delete this.Pr.Spacing.Line;
            ...
            case historyitem_Paragraph_Spacing_AfterAutoSpacing:
            {
                ...
                if ( null != Old )
                {
                    ...
                    delete this.Pr.Spacing.AfterAutoSpacing;
            ...
            case historyitem_Paragraph_Spacing_BeforeAutoSpacing:
            {
                ...
                if ( null != Old )
                {
                    ...
                    delete this.Pr.Spacing.BeforeAutoSpacing;
            ...
            case historyitem_Paragraph_Shd_Value:
            {
                ...
                else if ( "undefined" != typeof(this.Pr.Shd) )
                    delete this.Pr.Shd.Value;
            ...
            case historyitem_Paragraph_Style:
            {
                var Old = Data.Old;
                if ( null != Old )
                    this.ParaStyle = Old;
        ...
        this.Recalculate();//TODO:
        if(this.Parent.Parent.txBody)
        {
            this.Parent.Parent.txBody.recalculate();
        }
    ...
    createCopy : function(parent)
    {
        ...
        if(this.level != null)
        {
            _copy_para.level = this.level;
        }
        _copy_para.Pr = clone(this.Pr);
...
//-----------------------------------------------------------------------------------
    Document_Is_SelectionLocked : function(CheckType)
    {
        switch ( CheckType )
        {
            ...
            {
                // Если у нас нет выделения, и курсор стоит в начале, мы должны проверить предыдущий элемент
                ...
                // Если есть выделение, и знак параграфа попал в выделение
                else if ( true === this.Selection.Use )
                {
                    var StartPos = this.Selection.StartPos;
                    var EndPos   = this.Selection.EndPos;
    ...
    Save_Changes : function(Data, Writer)
    {
        ...
        switch ( Type )
        {
            case  historyitem_Paragraph_AddItem:
            {
                ...
                Writer.WriteLong( Data.Items.length );
            ...
            case historyitem_Paragraph_RemoveItem:
            {
                // Long : Начальная позиция
                // Long : Конечная позиция

                Writer.WriteLong( Data.Pos );
                Writer.WriteLong( Data.EndPos );
            ...
            case historyitem_Paragraph_Numbering:
            {
                ...
                else
                {
                    ...
                    Writer.WriteString2( Data.New.NumId );
                    Writer.WriteLong( Data.New.Lvl );
                }
            ...
            case historyitem_Paragraph_Tabs:
            {
                Styles_Write_Tabs_ToBinary( Writer, Data.New );
            ...
            case historyitem_Paragraph_Borders_Top:
            {
                Styles_Write_Brd_ToBinary( Writer, Data.New );
                break;
            }
            ...
            case historyitem_Paragraph_Pr:
            {
                ...
                {
                    Writer.WriteBool( false );
                    Styles_Write_ParaPr_ToBinary( Writer, Data.New );
                }
    ...
    Load_Changes : function(Reader)
    {
        ...
        switch ( Type )
        {
            case  historyitem_Paragraph_AddItem:
            {
                ...
                var RealPos = this.Internal_Get_RealPos( Pos );

                var Array_start = this.Content.slice( 0, RealPos );
                var Array_end   = this.Content.slice( RealPos );

                this.Content = Array_start.concat( Items, Array_end );

                this.Content.splice( RealPos + Items.length, 0, new ParaCollaborativeChangesEnd() );
                this.Content.splice( RealPos, 0, new ParaCollaborativeChangesStart() );
            ...
            case historyitem_Paragraph_RemoveItem:
            {
                ...
                var StartPos = this.Internal_Get_RealPos( Reader.GetLong() );
                var EndPos   = this.Internal_Get_RealPos( Reader.GetLong() );
            ...
            case historyitem_Paragraph_Ind_First:
            {
                ...
                if ( true === bDelete )
                {
                    if ( "undefined" != typeof(this.Pr) && "undefined" != typeof(this.Pr.Ind) )
                        delete this.Pr.Ind.FirstLine;
                }
                else
                {
            ...
            case historyitem_Paragraph_Shd_Color:
            {
                ...
                if ( false === bDelete )
                {
                    ...
                    this.Pr.Shd.Color.b = Reader.GetByte();
                }
                else if ( "undefined" != typeof(this.Pr.Shd) )
                    delete this.Pr.Shd.Color;
            ...
            case historyitem_Paragraph_Tabs:
            {
                var Tabs = Styles_Read_Tabs_FromBinary( Reader );
                if ( null != Tabs )
                    this.Pr.Tabs = Tabs;
                else
                    delete this.Pr.Tabs;
            ...
            case historyitem_Paragraph_Style:
            {
                ...
                var bDelete = Reader.GetBool();
                if ( false === bDelete )
                    this.ParaStyle = Reader.GetString2();
                else
                    delete this.ParaStyle;
    ...
    Write_ToBinary2 : function(Writer)
    {
        ...
        // Bool     : Задан ли стиль
        //   String   : StyleId (если предыдущее true)
        // Bool     : Задана ли нумерация
        //   String   : NumPr.NumId
        //   Byte     : NumPr.Lvl
        // Variable : ParaPr
        // String   : Id TextPr
        ...
        {
            Writer.WriteBool( true );
            Writer.WriteString2( "" + NumPr.NumId );
            Writer.WriteByte( NumPr.Lvl );
        }
        else
            Writer.WriteBool( false );

        Styles_Write_ParaPr_ToBinary( Writer, this.Pr );
    ...
    {
        if ( "undefined" != typeof(LinkData.Next) )
            this.Next   = g_oTableId.Get_ById( LinkData.Next );

        if ( "undefined" != typeof(LinkData.Prev) )
