...
{
    this.m_bFirst_Col = false;
    this.m_bFirst_Row = false;
    this.m_bLast_Col  = false;
    this.m_bLast_Row  = false;
    this.m_bBand_Hor  = false;
    this.m_bBand_Ver  = false;

    this.Set = function(bFC, bFR, bLC, bLR, bBH, bBV)
    {
    ...
    this.Is_FirstCol = function()
    {
        return this.m_bFirst_Col;
    };

    this.Is_FirstRow = function()
    {
        return this.m_bFirst_Row;
    };

    this.Is_LastCol = function()
    {
        return this.m_bLast_Col;
    };

    this.Is_LastRow = function()
    {
        return this.m_bLast_Row;
    };

    this.Is_BandHor = function()
    {
        return this.m_bBand_Hor;
    };

    this.Is_BandVer = function()
    {
    ...
    this.Set( bFC, bFR, bLC, bLR, bBH, bBV );
};
...
function CTable(DrawingDocument, Parent, Inline, PageNum, X, Y, XLimit, YLimit, Rows, Cols, TableGrid)
{
    ...
    this.TableLook  = new CTableLook(false, true, false, false, true, false);

    this.TableGrid = TableGrid;
    this.TableSumGrid = []; // данный массив будет заполнен после Internal_RecalculateGrid
    this.TableGridCalculated = TableGrid;
    ...
    this.paragraphStyles = new Array(9);
    for(var _style_level = 0; _style_level < 9; ++_style_level)
    {
        this.paragraphStyles[_style_level] = null;
    }
...
//-----------------------------------------------------------------------------------
    ...
    {
        for(var _index = 0; _index < this.Content.length; ++_index)
        {
            var _cur_row = this.Content[_index];
            for(var i = 0; i < _cur_row.Content.length; ++i)
            {
                var _cur_cell = _cur_row.Content[i];
                var _cur_par_arr = _cur_cell.Content.Content;
                for(var j = 0; j < _cur_par_arr.length; ++j)
                {
                    _cur_par_arr.calculateTextTheme(Theme, slide, Layout, Master, fontRef);
                }
            }
        }
    },      */
    hitToHyperlink: function(x, y)
    {
        ...
        var _cell_index
        var _cells_count;
        for (_row_index = 0; _row_index < _rows_count; ++_row_index)
        {
            _cells = _rows[_row_index].Content;
            _cells_count = _cells.length;
            for(_cell_index = 0; _cell_index < _cells_count; ++_cell_index)
            {
                if(_cells[_cell_index].Content.hitToHyperlink(x, y) === true)
                    return true;
            }
        }
        return false;
    ...
    {
        var _theme, _slide, _layout, _master;
        var _graphic_frame = this.Parent;
        switch(_graphic_frame.parent.kind)
        {
            case SLIDE_KIND:
            {
                _slide = _graphic_frame.parent;
        ...
        for(_row_index = 0; _row_index < _rows_count; ++_row_index)
        {
            _rows[_row_index].calculateTextTheme(_theme, _slide, _layout, _master)
        }
    ...
    calculateNumbering: function()
    {
        ...
        for(_row_index = 0; _row_index < _rows_count; ++_row_index)
        {
            _rows[_row_index].calculateNumbering();
        }
    ...
    Get_Props : function()
    {
        ...
        if ((this.Selection.Type === table_Selection_Cell && true === this.Selection.Use) || !(table_Selection_Border === this.Selection.Type2) && !(table_Selection_Border_InnerTable === this.Selection.Type2))
        {
            if((this.Selection.Type === table_Selection_Cell && true === this.Selection.Use))
            {
                ...
                {
                    Pr.CellsBackground = {fill: CreateAscFill(_compared_unifill)};
                }
            ...
            else if(!(table_Selection_Border === this.Selection.Type2) && !(table_Selection_Border_InnerTable === this.Selection.Type2))
            {
                Pr.CellSelect = true;
    ...
    Copy : function(Parent)
    {
        ...
        // Копируем настройки
        Table.Pr         = this.Pr.Copy();
        Table.TableStyle = this.TableStyle;
        Table.TableLook  = this.TableLook.Copy();
        ...
        Table.styleIndex = this.styleIndex;
        Table.Recalculate();
        Table.Recalc_CompiledPr();
        Table.CurCell = Table.Content[0].Content[0];
        return Table;
    },
    ...
    {
        if(this.CurCell !== null && typeof this.CurCell === "object" && typeof this.CurCell.canIncreaseIndent === "function")
        {
            return this.CurCell.canIncreaseIndent(bIncrease);
        }
        return false;
    },
    ...
    // Запрашиваем новую позицию для содержимого у текущей ячейки
    Get_PageContentStartPos : function(PageNum, RowIndex, CellIndex, bFromCell)
    {
        ...
        // TODO: Переделать пересчет StartPage
        var StartPage = Cell.Temp.CurPage + PageNum;

        // Если оказалось, что данная строка не та, на которой был рассчитан перенос, тогда пересчитываем заново
        if ( "undefined" != typeof(this.Pages[StartPage]) && ( RowIndex + VMerge_count - 1 >= this.Pages[StartPage].FirstRow && RowIndex <= this.Pages[StartPage].FirstRow && RowIndex ) )
        {
            return { X : CellInfo.X_content_start, XLimit : CellInfo.X_content_end, Y : this.Pages[StartPage].Y, YLimit : this.Pages[StartPage].YLimit, MaxTopBorder : this.Pages[StartPage].MaxTopBorder };
        }
        ...
        var Pos = this.Parent.Get_PageContentStartPos();

        // На момент обращения к данной функции, у всех ячеек всех строк до
        // текущей (включительно) должны быть просчитаны верхние границы.
...
//-----------------------------------------------------------------------------------
    ...
    Move : function(X, Y, PageNum)
    {
        ...
        if ( true != this.Is_Inline() )
        {
            if ( false === oLogicDocument.Document_Is_SelectionLocked(changestype_Table_Properties) )
            {
                ...
                this.Parent.Update_Position2( this.X, this.Y, PageNum );

                this.CurCell = this.Content[0].Get_Cell(0);
                this.CurCell.Content.Cursor_MoveToStartPos();
                this.Internal_Update_TableMarkup( 0, 0, 0 );
            }
        }
        else
        {
            ...
            {
                oLogicDocument.Create_NewHistoryPoint();
                this.Parent.InlineTable_Move( this, X, Y, PageNum);
            }
        }
    },
    ...
    Draw : function(nPageIndex, pGraphics)
    {
        ...
        // Сначала заливаем и обводим общую рамку таблицы
        ...
        for ( var CurRow = Row_start; CurRow <= Row_last; CurRow++ )
        {
            ...
            if ( shd_Nil != TableShd.Value )
            {
                ...
                /*
                pGraphics._s();
                pGraphics._m( X_left_new  * 100, Y_top    * 100 );
                pGraphics._l( X_right_new * 100, Y_top    * 100 );
                pGraphics._l( X_right_new * 100, Y_bottom * 100 );
                pGraphics._l( X_left_new  * 100, Y_bottom * 100 );
                pGraphics._l( X_left_new  * 100, Y_top    * 100 );
                pGraphics.df();
                */
                pGraphics.TableRect(Math.min(X_left_new, X_right_new), Math.min(Y_top, Y_bottom),
                    Math.abs(X_right_new - X_left_new), Math.abs(Y_bottom - Y_top));
            }
            ...
            if ( null != CellSpacing )
            {
                ...
                if ( border_Single === TableBorders.Left.Value )
                {
                    ...
                    //pGraphics.p_width( TableBorders.Left.Size * 1000 );
                    //pGraphics._s();
                    if ( Row_start === CurRow || Math.abs( X_left_new - X_left_old ) < 0.001 )
                    {
                    ...
                    else
                    {
                        if ( X_left_new > X_left_old )
                        {
                            ...
                            //pGraphics._m( ( X_left_old - TableBorders.Left.Size / 2 ) * 100 , ( Y_top + TableBorders.Left.Size / 2) * 100 );
                            //pGraphics._l( X_left_new * 100,                                   ( Y_top + TableBorders.Left.Size / 2) * 100 );
                        }
                        else
                        {
                ...
                if ( border_Single === TableBorders.Right.Value )
                {
                    ...
                    //pGraphics.p_width( TableBorders.Right.Size * 1000 );
                    //pGraphics._s();
                    if ( Row_start === CurRow || Math.abs( X_right_new - X_right_old ) < 0.001 )
                    {
                        pGraphics.drawVerLine( c_oAscLineDrawingRule.Center, X_right_new, Y_top, Y_bottom, TableBorders.Right.Size );
                        //pGraphics._m( X_right_new * 100, Y_top    * 100 );
                        //pGraphics._l( X_right_new * 100, Y_bottom * 100 );
                    }
                    else
                    {
                        if ( X_right_new > X_right_old )
                        {
                            ...
                            //pGraphics._m( ( X_right_old - TableBorders.Right.Size / 2 ) * 100 , ( Y_top - TableBorders.Right.Size / 2 ) * 100 );
                            //pGraphics._l( X_right_new * 100,                                    ( Y_top - TableBorders.Right.Size / 2 ) * 100 );
                        }
                        else
                        {
                ...
                if ( Row_start === CurRow )
                {
                    ...
                    if ( border_Single === TableBorders.Top.Value )
                    {
                        ...
                        //pGraphics.p_width( TableBorders.Top.Size * 1000 );
                        //pGraphics._s();
                        ...
                        //if ( border_Single === TableBorders.Left.Value )
                        //    pGraphics._m( ( X_left_new - TableBorders.Left.Size / 2 ) * 100, ( Y_top + TableBorders.Top.Size / 2 ) * 100 );
                        //else
                        //    pGraphics._m( X_left_new  * 100, ( Y_top + TableBorders.Top.Size / 2 ) * 100 );

                        //if ( border_Single === TableBorders.Right.Value )
                        //    pGraphics._l( (X_right_new + TableBorders.Right.Size / 2 ) * 100, ( Y_top + TableBorders.Top.Size / 2 ) * 100 );
                        //else
                        //    pGraphics._l( X_right_new * 100, ( Y_top + TableBorders.Top.Size / 2 ) * 100 );
                ...
                if ( Row_last === CurRow )
                {
                    ...
                    if ( border_Single === TableBorders.Bottom.Value )
                    {
                        ...
                        //pGraphics.p_width( TableBorders.Bottom.Size * 1000 );
                        //pGraphics._s();
                        ...
                        //if ( border_Single === TableBorders.Left.Value )
                        //    pGraphics._m( ( X_left_new - TableBorders.Left.Size / 2 )  * 100, ( Y_bottom + TableBorders.Bottom.Size / 2 ) * 100 );
                        //else
                        //    pGraphics._m( X_left_new  * 100, ( Y_bottom + TableBorders.Bottom.Size / 2 ) * 100 );

                        //if ( border_Single === TableBorders.Right.Value )
                        //    pGraphics._l( ( X_right_new + TableBorders.Right.Size / 2 ) * 100, ( Y_bottom + TableBorders.Bottom.Size / 2 ) * 100 );
                        //else
                        //    pGraphics._l( X_right_new * 100, ( Y_bottom + TableBorders.Bottom.Size / 2 ) * 100 );
            ...
            for ( var CurCell = CellsCount - 1; CurCell >= 0; CurCell-- )
            {
                ...
                if ( shd_Nil != CellShd.Value )
                {
                    if ( null != CellSpacing )
                    {
                        ...
                        /*
                        pGraphics._s();
                        pGraphics._m( X_cell_start * 100, Y * 100 );
                        pGraphics._l( X_cell_end * 100, Y * 100);
                        pGraphics._l( X_cell_end * 100, ( Y + RealHeight ) * 100 );
                        pGraphics._l( X_cell_start * 100, ( Y + RealHeight ) * 100 );
                        pGraphics._l( X_cell_start * 100, Y * 100 );
                        pGraphics.df();
                        */
                        pGraphics.TableRect(Math.min(X_cell_start, X_cell_end), Math.min(Y, Y + RealHeight),
                            Math.abs(X_cell_end - X_cell_start), Math.abs(RealHeight));
                    }
                    else
                    {
                        pGraphics.b_color1( CellShd.Color.r, CellShd.Color.g, CellShd.Color.b, 255 );

                        var RightInfo = Cell.Get_BorderInfo().Right;
                        ...
                        var Row_side_border_end   = ( RightInfo.length - 1 + TempCurRow > Row_last ? Row_last - TempCurRow + 1 : RightInfo.length - 1 );
                        for ( var Index = Row_side_border_start; Index <= Row_side_border_end; Index++ )
                        {
                            var CurBorderInfo = RightInfo[Index];
                            ...
                            var X0 = X_cell_start;
                            var X1 = X_cell_end - CurBorderInfo.Size / 2;

                            if ( Row_side_border_start === Index && LastBorderTop.W > 0 && LastBorderTop.L > 0 )
                            {
                                /*
                                pGraphics._s();
                                pGraphics._m( X0 * 100, Y0 * 100 );
                                pGraphics._l( (X1 - LastBorderTop.L) * 100, Y0 * 100);
                                pGraphics._l( (X1 - LastBorderTop.L) * 100, (Y0 + LastBorderTop.W) * 100 );
                                pGraphics._l( X0 * 100, (Y0 + LastBorderTop.W) * 100 );
                                pGraphics._l( X0 * 100, Y0 * 100 );
                                pGraphics.df();

                                pGraphics._s();
                                pGraphics._m( X0 * 100, (Y0 + LastBorderTop.W) * 100 );
                                pGraphics._l( X1 * 100, (Y0 + LastBorderTop.W) * 100);
                                pGraphics._l( X1 * 100, Y1 * 100 );
                                pGraphics._l( X0 * 100, Y1 * 100 );
                                pGraphics._l( X0 * 100, (Y0 + LastBorderTop.W) * 100 );
                                pGraphics.df();
                                */

                                pGraphics.TableRect( X0, Y0, X1 - LastBorderTop.L - X0, LastBorderTop.W);
                                pGraphics.TableRect( X0, Y0 + LastBorderTop.W, X1 - X0, Y1 - Y0 - LastBorderTop.W );
                            }
                            else
                            {
                                /*
                                ...
                                */
                                ...
                                pGraphics.TableRect( X0, Y0, X1 - X0, Y1 - Y0 );
                            }
                        }
                    }
                }
                    ...
                    for ( var Index = 0; Index < BorderInfo_Top.length; Index++ )
                    {
                        ...
                        if ( 0 === Index )
                        {
                            ...
                            if( BorderInfo_Left.length > 0 && border_Single === BorderInfo_Left[0].Value && BorderInfo_Left[0].Size / 2 > Max_l )
                                Max_l = 0;//BorderInfo_Left[0].Size / 2;
        ...
        for ( var CurRow = Row_start; CurRow <= Row_last; CurRow++ )
        {
            ...
            for ( var CurCell = 0; CurCell < CellsCount; CurCell++ )
            {
                ...
                Cell.Content_Draw(nPageIndex - this.PageNum, pGraphics);
            }
        }
            ...
            for ( var CurCell = CellsCount - 1; CurCell >= 0; CurCell-- )
            {
                ...
                if ( null != CellSpacing )
                {
                    ...
                    if ( border_Single === CellBorders.Left.Value )
                    {
                        ...
                        //pGraphics.p_width( CellBorders.Left.Size * 1000 );
                        //pGraphics._s();

                        pGraphics.drawVerLine( c_oAscLineDrawingRule.Left, X_cell_start, Y, Y + RealHeight, CellBorders.Left.Size );
                        //pGraphics._m( ( X_cell_start + CellBorders.Left.Size / 2) * 100, Y * 100 );
                        //pGraphics._l( ( X_cell_start + CellBorders.Left.Size / 2) * 100, ( Y + RealHeight ) * 100 );
                    ...
                    if ( border_Single === CellBorders.Right.Value )
                    {
                        ...
                        //pGraphics.p_width( CellBorders.Right.Size * 1000 );
                        //pGraphics._s();

                        pGraphics.drawVerLine( c_oAscLineDrawingRule.Right, X_cell_end, Y, Y + RealHeight, CellBorders.Right.Size );
                        //pGraphics._m( (X_cell_end - CellBorders.Right.Size / 2) * 100, Y * 100 );
                        //pGraphics._l( (X_cell_end - CellBorders.Right.Size / 2) * 100, ( Y + RealHeight ) * 100 );
                    ...
                    if ( border_Single === CellBorders.Top.Value )
                    {
                        ...
                        //pGraphics.p_width( CellBorders.Top.Size * 1000 );
                        //pGraphics._s();

                        //pGraphics.drawHorLineExt( c_oAscLineDrawingRule.Bottom, Y, X_cell_start, X_cell_end, CellBorders.Top.Size, 0, 0 );
                        ...
                        //pGraphics._m( X_cell_start * 100, ( Y - CellBorders.Top.Size / 2 ) * 100 );
                        //pGraphics._l( X_cell_end   * 100, ( Y - CellBorders.Top.Size / 2 ) * 100 );
                    ...
                    if ( border_Single === CellBorders.Bottom.Value )
                    {
                        ...
                        //pGraphics.p_width( CellBorders.Bottom.Size * 1000 );
                        //pGraphics._s();

                        //pGraphics.drawHorLineExt( c_oAscLineDrawingRule.Top, Y + RealHeight, X_cell_start, X_cell_end, CellBorders.Bottom.Size, 0, 0 );
                        ...
                        //pGraphics._m( X_cell_start * 100, ( Y + RealHeight + CellBorders.Bottom.Size / 2 ) * 100 );
                        //pGraphics._l( X_cell_end   * 100, ( Y + RealHeight + CellBorders.Bottom.Size / 2 ) * 100 );
    ...
    Get_NearestPos : function( PageNum, X, Y )
    {
        ...
        return Cell.Content.Get_NearestPos( PNum, X, Y );
    },
    ...
    Update_CursorType : function(X, Y, PageIndex)
    {
     ...
     //   this.DrawingDocument.StartTrackTable( NewOutline );
    ...
    Document_UpdateInterfaceState : function()
    {
        ...
        if ( true != this.Selection.Use || table_Selection_Cell != this.Selection.Type )
        {
            ...
            editor.asc_fireCallback("asc_canIncreaseIndent", this.CurCell.Content.canIncreaseIndent(true));
            editor.asc_fireCallback("asc_canDecreaseIndent", this.CurCell.Content.canIncreaseIndent(false));
        }
        else
        {
            ...
            if(true === this.Selection.Use && table_Selection_Cell === this.Selection.Type)
            {
                var _selection_data = this.Selection.Data;
                var _selection_count = _selection_data.length;
                var _select_cell_index;
                var _selected_cell;
                var _pos_cell;
                var _cur_row;
                var _cur_cell_content;
                var _can_increase = true;
                var _can_decrease = true;
                for(_select_cell_index = 0; _select_cell_index < _selection_count; ++_select_cell_index)
                {
                    _pos_cell = _selection_data[_select_cell_index];
                    _cur_row= this.Content[_pos_cell.Row];
                    _selected_cell = _cur_row.Get_Cell( _pos_cell.Cell );
                    _cur_cell_content = _selected_cell.Content;
                    if(_can_increase === true && _cur_cell_content.canIncreaseIndent(true) === false)
                    {
                        _can_increase = false;
                        editor.asc_fireCallback("asc_canIncreaseIndent", false);
                    }
                    if(_can_decrease === true && _cur_cell_content.canIncreaseIndent(false) === false)
                    {
                        _can_decrease = false;
                        editor.asc_fireCallback("asc_canDecreaseIndent", false);
                    }

                    if(_can_increase === false && _can_decrease === false)
                    {
                        break;
                    }
                }

                if(_can_increase === true)
                {
                    editor.asc_fireCallback("asc_canIncreaseIndent", true);
                }
                if(_can_decrease === true)
                {
                    editor.asc_fireCallback("asc_canDecreaseIndent", true);
                }
            }
            else
            {
    ...
    // Проверяем, можно ли добавить данную таблицу
    Check_AddTable : function(Table)
    {
        if ( this === Table )
            return false;

        return this.CurCell.Content.Check_AddTable( Table );
    },

    Add_InlineTable2 : function(Table)
    {
        this.CurCell.Content.Add_InlineTable2( Table );
    },
...
//-----------------------------------------------------------------------------------
    Undo : function(Data)
    {
        ...
        switch ( Type )
        {
            ...
            {
                if(typeof Data.undo_function === "function")
                {
    ...
    Redo : function(Data)
    {
        ...
        switch ( Type )
        {
            case history_undo_redo_const:
            {
                if(typeof Data.redo_function === "function")
                {
...
//-----------------------------------------------------------------------------------
    ...
    {
        if ( false === this.Selection.Use || ( true === this.Selection.Use && table_Selection_Text === this.Selection.Type ) )
            return this.CurCell.Content.Hyperlink_CanAdd();
...
//-----------------------------------------------------------------------------------
    ...
    Selection_SetEnd : function( X, Y, PageIndex, MouseEvent )
    {
        ...
        if ( this.Selection.Type2 === table_Selection_Border )
        {
            ...
            var _pos_x = parseInt(this.Parent.TransformMatrix.TransformPointX(X, Y) / 2.5 + 0.5) * 2.5;
            var _pos_y = parseInt(this.Parent.TransformMatrix.TransformPointY(X, Y) / 2.5 + 0.5) * 2.5;
            ...
            if ( true === this.Selection.Data2.bCol )
            {
                ...
                {
                    _X = Math.max( _X, this.Selection.Data2.Min );
                    _pos_x = Math.max(_pos_x, parseInt(this.Parent.TransformMatrix.TransformPointX(this.Selection.Data2.Min, Y)/2.5 + 0.5)*2.5);
                }

                if ( null != this.Selection.Data2.Max )
                {
                    _X = Math.min( _X, this.Selection.Data2.Max );
                    _pos_x = Math.min(_pos_x, parseInt(this.Parent.TransformMatrix.TransformPointX(this.Selection.Data2.Max, Y)/2.5 + 0.5)*2.5);
                }
            ...
            else
            {
                ...
                {
                    _Y = Math.max( _Y, this.Selection.Data2.Min );
                    _pos_y = Math.max(_pos_y, parseInt(this.Parent.TransformMatrix.TransformPointY(X, this.Selection.Data2.Min)/2.5 +0.5)*2.5);
                ...
                {
                    _Y = Math.min( _Y, this.Selection.Data2.Max );
                    _pos_y = Math.min(_pos_y, parseInt(this.Parent.TransformMatrix.TransformPointY(X, this.Selection.Data2.Max)/2.5 + 0.5)*2.5);
                }
                        ...
                        if ( 0 === Col )
                        {
                            ...
                            if ( BeforeSpace2 < 0 )
                            {
                                ...
                                if ( true === this.Is_Inline() )
                                    ...
                                    this.Parent.Update_Position( this.X, this.Y );
                            }
                        }
            ...
            if(typeof this.Parent.RecalculateSize === "function")
            {
    ...
    {
        if(this.Parent && this.Parent.TransformMatrix)
            this.DrawingDocument.UpdateTargetTransform(this.Parent.TransformMatrix);
        if ( this.Selection.Type2 === table_Selection_Border )
        {
            if ( true === this.Selection.Data2.bCol )
            {
    ...
    Selection_IsEmpty : function()
    {
        if ( true === this.Selection.Use )
        {
            if ( table_Selection_Cell === this.Selection.Type )
                ...
                return this.CurCell.Content.Selection_IsEmpty();
        }
...
//-----------------------------------------------------------------------------------
    ...
    Add_InlineImage : function(W, H, Img)
    {
        this.Selection.Use  = true;
        this.Selection.Type = table_Selection_Text;
        this.CurCell.Content.Add_InlineImage(W,H,Img);
    },
    ...
    Add_InlineObject : function(Obj)
    {
        ...
        this.CurCell.Content.Add_InlineObject(Obj);
    },
    ...
    Paragraph_Add : function( ParaItem, bRecalculate )
    {
        if ( para_TextPr === ParaItem.Type && ( true === this.ApplyToAll || ( true === this.Selection.Use && table_Selection_Cell === this.Selection.Type && this.Selection.Data.length > 0 ) ) )
        {
            var _history_obj;
            var Cells_array = this.Internal_Get_SelectionArray();
            var _check_need_recalc = ParaItem.Value.Check_NeedRecalc();
            if(_check_need_recalc === true)
            {
                _history_obj = {};
                if ( Cells_array[0].Row - 1 >= 0 )
                {
                    _history_obj.rowIndex = Cells_array[0].Row - 1;
                    _history_obj.undo_function = function(data)
                    {
                        this.Internal_RecalculateFrom( data.rowIndex, 0, true, true );
                    };
                ...
                {
                    _history_obj.rowIndex = this.Index;
                    _history_obj.undo_function = function(data)
                    {
                        this.Internal_Recalculate_1();
                        this.Internal_OnContentRecalculate( true, 0, data.rowIndex );
                    };
                    _history_obj.redo_function = function(data)
                    {
                    };
        ...
        else
        {
            if(this.Selection.Use === true && table_Selection_Cell === this.Selection.Type )
            {
                ...
                this.Selection.StartPos.X          = 0;
                this.Selection.StartPos.Y          = 0;
                this.Selection.StartPos.PageIndex  = 0;
                this.Selection.StartPos.MouseEvent =
                {
                    // TODO : Если в MouseEvent будет использоваться что-то кроме ClickCount, Type и CtrlKey, добавить здесь
    ...
    {
        return this.CurCell.Content.Get_ParagraphIndent();
    },
...
//-----------------------------------------------------------------------------------
    ...
    Recalc_CompiledPr : function()
    {
        ...
        {
            _rows[_row_index].Recalc_CompiledPr();
        }
    },
    ...
    {
        var Styles =  this.Get_Styles(), StyleId;
        ...
        {
            if((Pr.TextPr.fontRef !== null && typeof Pr.TextPr.fontRef === "object")
                || (Pr.TextPr.unifill !== null && typeof Pr.TextPr.unifill === "object"
                && Pr.TextPr.unifill.fill !== null && typeof Pr.TextPr.unifill.fill === "object"))
            {
            ...
            if((Pr.TextPr.fontRef !== null && typeof Pr.TextPr.fontRef === "object" && typeof Pr.TextPr.fontRef.idx === "number"))
            {
                switch  (Pr.TextPr.fontRef.idx)
                {
                    ...
                    {
                        Pr.TextPr.FontFamily = {Name : getFontInfo("+mj-lt")(_theme.themeElements.fontScheme), Index : -1};
                        break;
                    }
                    case fntStyleInd_minor :
                    {
                        Pr.TextPr.FontFamily = {Name : getFontInfo("+mn-lt")(_theme.themeElements.fontScheme), Index : -1};
                        break;
                    }
                    default :
                    {
        ...
        for(var key in Pr)
        {
            if(Pr[key] instanceof  CTableStylePr)
            {
                ...
                {
                    if((_text_pr.fontRef !== null && typeof _text_pr.fontRef === "object")
                        || (_text_pr.unifill !== null && typeof _text_pr.unifill === "object"
                        && _text_pr.unifill.fill !== null && typeof _text_pr.unifill.fill === "object"))
                    {
                    ...
                    if((_text_pr.fontRef !== null && typeof _text_pr.fontRef === "object" && typeof _text_pr.fontRef.idx === "number"))
                    {
                        switch  (_text_pr.fontRef.idx)
                        {
                            case fntStyleInd_major :
                            {
                                _text_pr.FontFamily = {Name : getFontInfo("+mj-lt")(_theme.themeElements.fontScheme), Index : -1};
                                break;
                            }
                            case fntStyleInd_minor :
                            {
                                _text_pr.FontFamily = {Name : getFontInfo("+mn-lt")(_theme.themeElements.fontScheme), Index : -1};
                                break;
                            }
                            default :
                            {
                ...
                {
                    if(_table_cell_pr.Shd !== null && typeof _table_cell_pr.Shd === "object"
                        && _table_cell_pr.Shd.unifill !== null && typeof _table_cell_pr.Shd.unifill === "object"
                        && _table_cell_pr.Shd.unifill.fill !== null && typeof _table_cell_pr.Shd.unifill.fill === "object")
                    {
                        _calculated_unifill = _table_cell_pr.Shd.unifill.createDuplicate();
                        _calculated_unifill.calculate(_theme, _slide, _layout, _master, {R:0, G:0, B: 0, A:255});
                    ...
                    if(_table_cell_pr.TableCellBorders !== null && typeof _table_cell_pr.TableCellBorders === "object")
                    {
                        ...
                        for( key in _borders)
                        {
                            if(_borders[key] instanceof  CDocumentBorder)
                            {
                                ...
                                if( (_calculated_line.Fill !== null && typeof _calculated_line.Fill === "object"
                                    && _calculated_line.Fill.fill !== null && typeof _calculated_line.Fill.fill === "object"))
                                {
       ...
       /* if(Pr.TableCellPr !== null && typeof Pr.TableCellPr === "object")
        {
            if(Pr.TableCellPr.Shd !== null && typeof Pr.TableCellPr.Shd === "object"
                && Pr.TableCellPr.Shd.unifill !== null && typeof Pr.TableCellPr.Shd.unifill === "object"
                && Pr.TableCellPr.Shd.unifill.fill !== null && typeof Pr.TableCellPr.Shd.unifill.fill === "object")
            {
                _calculated_unifill = Pr.TableCellPr.Shd.unifill.createDuplicate();
                _calculated_unifill.calculate(_theme, _slide, _layout, _master, {R:0, G:0, B: 0, A:255});
                if(_calculated_unifill.fill.color && _calculated_unifill.fill.color.RGBA !== null && typeof _calculated_unifill.fill.color.RGBA === "object")
                {
                    RGBA = _calculated_unifill.fill.color.RGBA;
                    Pr.TableCellPr.Shd.Color = new CDocumentColor(RGBA.R, RGBA.G, RGBA.B);
                }
            }

            if(Pr.TableCellPr.TableCellBorders !== null && typeof Pr.TableCellPr.TableCellBorders === "object")
            {
                var _borders = Pr.TableCellPr.TableCellBorders;
                var _border;
                for( key in _borders)
                {
                    if(_borders[key] instanceof  CDocumentBorder)
...
//-----------------------------------------------------------------------------------
    Clear_DirectFormatting : function()
    {
        ...
        for ( var Index = 0; Index < Count; Index++ )
        {
            this.Content[Index].Clear_DirectFormatting();
        }
    },
    ...
    Set_TableStyle : function(StyleId)
    {
        ...
        {
            History.Add( this, { Type : historyitem_Table_TableStyle, Old : this.styleIndex, New : StyleId } );
            this.styleIndex = StyleId;

            // Очищаем все прямое форматирование таблицы
            this.Clear_DirectFormatting();
...
//-----------------------------------------------------------------------------------
    ...
    Cell_Merge : function()
    {
        ...
        */
        ...
        this.Internal_Check_TableRows();
        //if ( true === this.Internal_Check_TableRows() )
    ...
    Cell_Split : function(Rows, Cols)
    {
            ...
            else
            {
                ...
                for ( var Index = 1; Index < Rows; Index++ )
                {
                    ...
                    for ( var CurCell = 0; CurCell < CellsCount; CurCell++ )
                    {
                        ...
                        var _first_text_pr = Old_Cell.Content.getFirstTextProperties();
                        var _first_parapr_pr = Old_Cell.Content.getFirstParagraphProperties();
                        New_Cell.Copy_Pr( Old_Cell.Pr );
                        if(_first_text_pr !== null)
                            New_Cell.Content.setFirstTextProperties(_first_text_pr);
                        if(_first_parapr_pr !== null)
                            New_Cell.Content.setFirstParagraphProperties(_first_parapr_pr);
        ...
        if ( Cols > 1 )
        {
            ...
            for ( var Index = 0; Index < this.TableGrid.length; Index++ )
                Grid_Info_start[Index] = this.TableGrid[Index];
            ...
            for ( var Index2 = 0; Index2 < Rows_.length; Index2++ )
            {
                if ( null != Cells[Index2] && null != Cells_pos[Index2] )
                {
                    ...
                    var _first_text_pr = TempCell.Content.getFirstTextProperties();
                    var _first_parapr_pr = TempCell.Content.getFirstParagraphProperties();
                    ...
                    for ( var Index = 1; Index < Cols; Index++ )
                    {
                        ...
                        if(_first_text_pr !== null)
                            NewCell.Content.setFirstTextProperties(_first_text_pr);
                        if(_first_parapr_pr !== null)
                            NewCell.Content.setFirstParagraphProperties(_first_parapr_pr);
                    }
                }
            }
            ...
            var OldTableGridLen = this.TableGrid.length;
            ...
            {
                var Summary = this.TableGrid[Index];
    ...
    Col_Add : function(bBefore)
    {
        ...
        if ( true === bBefore )
        {
            ...
            for ( var CurRow = 0; CurRow < this.Content.length; CurRow++ )
            {
                ...
                for ( var Index = 0; Index < Count; Index++ )
                {
                    Row.Add_Cell( Add_info[CurRow], Row, null, false, Row.Get_Cell( Add_info[CurRow]));
        ...
        else
        {
            ...
            for ( var CurRow = 0; CurRow < this.Content.length; CurRow++ )
            {
                ...
                for ( var Index = 0; Index < Count; Index++ )
                {
                    Row.Add_Cell( Add_info[CurRow] + 1, Row, null, false, Row.Get_Cell( Add_info[CurRow]) );
...
//-----------------------------------------------------------------------------------
    ...
    {
        if ( true != this.TableGridNeedRecalc )
            return;
        ...
        for ( var CurRow = 0; CurRow < this.Content.length; CurRow++  )
        {
            ...
            for ( var CurCell = 0; CurCell < CellsCount; CurCell++ )
            {
                ...
                */
                ...
                Cell.Content_Reset( X_content_start, Y_content_start, X_content_end, /*Y_content_end*/20000 );
                Cell.Recalculate();
    ...
    Internal_GetCellByXY : function( X, Y, PageIndex)
    {
        ...
        if ( X >= this.X )
        {
            for ( CurGrid = 0; CurGrid < this.TableGrid.length; CurGrid++ )
            {
        ...
        if ( CurGrid >= this.TableGrid.length )
            CurGrid = this.TableGrid.length - 1;
    ...
    Internal_Check_TableRows : function()
    {
        ...
        for ( var Index = 0; Index < Rows_to_CalcH.length; Index++ )
        {
            ...
            _history_data = {};
            _history_data.Type = history_undo_redo_const;
            _history_data.Pr = this.Content[RowIndex].Pr;
            _history_data.oldHeight = this.Content[RowIndex].Pr.Height;
            this.Content[RowIndex].Pr.Height = new CTableRowHeight(this.RowsInfo[RowIndex].H, heightrule_AtLeast);
            _history_data.newHeight = this.Content[RowIndex].Pr.Height;
            _history_data.undo_function = function(data)
            {
                data.Pr.Height = data.oldHeight;
            }
            _history_data.redo_function = function(data)
            {
                data.Pr.Height = data.newHeight;
            }
            History.Add(this, _history_data);
        }
        ...
        for ( var Counter = 0; Counter < Rows_to_Delete.length; )
        {
            ...
            var Summary_Height = this.RowsInfo[StartRow - 1 + CurRowSpan].H[0] + this.RowsInfo[StartRow - 1 + CurRowSpan].Y[0] - this.RowsInfo[StartRow - 1].Y[0];
            _history_data = {};
            _history_data.Type = history_undo_redo_const;
            _history_data.Pr = this.Content[StartRow - 1].Pr;
            _history_data.oldHeight = this.Content[StartRow - 1].Pr.Height;
            this.Content[StartRow - 1].Pr.Height =  new CTableRowHeight(Summary_Height, heightrule_AtLeast);
            _history_data.newHeight = this.Content[StartRow - 1].Pr.Height;
            _history_data.undo_function = function(data)
            {
                data.Pr.Height = data.oldHeight;
            }
    ...
    Internal_Update_TableMarkup : function(RowIndex, CellIndex, PageNum)
    {
        ...
        var _x = this.Parent.TransformMatrix.TransformPointX(0, 0);
        var _y = this.Parent.TransformMatrix.TransformPointY(0, 0);
    ...
    Internal_OnContentRecalculate : function( bNeedDocumentRecalc, PageNum, DocumentIndex )
    {
        ...
        {
            this.Parent.OnContentRecalculate( bNeedDocumentRecalc, PageNum, DocumentIndex, this );
        }
    },
    ...
    {
        var SumGrid = new Array();
        for (var Index = -1; Index < this.TableGrid.length; Index++ )
            SumGrid[Index] = 0;
...
CTableRow.prototype =
{
    ...
    calculateTextTheme: function(theme, slide, layout, master)
    {
        var _cells = this.Content;
        var _cell_index;
        var _cell_count = _cells.length;
        for(_cell_index = 0; _cell_index < _cell_count; ++_cell_index)
        {
            _cells[_cell_index].calculateTextTheme(theme, slide, layout, master);
        }
    },
    ...
    calculateNumbering: function()
    {
        ...
        for(_cell_index = 0; _cell_index < _cell_count; ++_cell_index)
        {
            _cells[_cell_index].calculateNumbering();
        }
    },
...
//-----------------------------------------------------------------------------------
    Recalc_CompiledPr : function()
    {
        ...
        var _cells = this.Content;
        var _cell_index;
        var _cells_count = _cells.length;
        for(_cell_index = 0; _cell_index < _cells_count; ++_cell_index)
        {
            _cells[_cell_index].Recalc_CompiledPr();
        }
    },
    ...
    Internal_Compile_Pr : function()
    {
        ...
        if ( true === TableLook.Is_FirstRow() && 0 === CurIndex  )
        {
...
//-----------------------------------------------------------------------------------
    ...
    Add_Cell : function(Index, Row, Cell, bReIndexing, cell)
    {
        if ( "undefined" === typeof(Cell) || null === Cell )
            Cell = new CTableCell( Row );
        if(cell instanceof CTableCell)
        {
            Cell.Pr.Set_FromObject(cell.Pr);
            var _first_text_properties = cell.getFirstTextProperties();
            if(_first_text_properties != null)
            {
                Cell.setFirstTextProperties(cell.getFirstTextProperties());
            }
            Cell.setFirstParagraphProperties(cell.getFirstParagraphProperties());
...
//-----------------------------------------------------------------------------------
    Undo : function(Data)
    {
        ...
        History.RecalcData_Add( this.Table);
    },
...
function CTableCell(Row)
{
    ...
    this.Container = this.Row.Table;
    this.parent = this.Container.Parent.parent;
    this.textStyles = new Array(9);
    for(var _style_index = 0; _style_index < 9; ++_style_index)
    {
        this.textStyles[_style_index] = null;
    }
    // Добавляем данный класс в таблицу Id (обязательно в конце конструктора)
    g_oTableId.Add( this, this.Id );
}
...
CTableCell.prototype =
{
    Copy : function(Row)
    {
        ...
        Cell.Pr = this.Pr.Copy();

        // Копируем содержимое ячейки
       // Cell.Content = this.Content.Copy(this);

        var _history_is_on = History.Is_On();
        if(_history_is_on)
            History.TurnOff();

        Cell.Content = new CDocumentContent(this, editor.WordControl.m_oLogicDocument.DrawingDocument, 0, 0, 0, 0, false, false);
        var _par_index;
        Cell.Content.Content.length = 0;
        for(_par_index = 0; _par_index < this.Content.Content.length; ++_par_index)
        {
            Cell.Content.Internal_Content_Add(_par_index, this.Content.Content[_par_index].createCopy(Cell.Content));
        }
    ...
    calculateTextTheme: function(theme, slide, layout, master)
    {
        var _paragraphs = this.Content.Content;
        var _paragraphs_count = _paragraphs.length;
        var _paragraph_index;
        for(_paragraph_index = 0; _paragraph_index < _paragraphs_count; ++_paragraph_index)
        {
            _paragraphs[_paragraph_index].calculateTextTheme(theme, slide, layout, master, null);
            _paragraphs[_paragraph_index].RecalcInfo.Set_Type_0(pararecalc_0_All);
        }
    },
    ...
    {
        return this.Content.getFirstTextProperties();
    },

    setFirstTextProperties: function(properties)
    {
        this.Content.setFirstTextProperties(properties)
    },

    getFirstParagraphProperties: function()
    {
        return this.Content.getFirstParagraphProperties();
    },

    setFirstParagraphProperties: function(paragraphProperties)
    {
        this.Content.setFirstParagraphProperties(paragraphProperties);
    },

    getParagraphStyles: function(level)
    {

        return this.Row.Table.getParagraphStyles(level);
    },

    calculateText3: function(par)
    {
        var _slide = null, _layout = null, _master = null, _theme = null;
        var _major_parent = this.Row.Table.Parent.parent;
        switch(_major_parent.kind)
        {
            case SLIDE_KIND :
            {
                _slide = _major_parent;
                _layout = _slide.Layout;
                if(_layout)
                {
                    _master = _layout.Master;
            ...
            case LAYOUT_KIND:
            {
                _layout = _major_parent;
                _master = _layout.Master;
                break;
            }
            case MASTER_KIND :
            {
                _master = _major_parent;
                break;
            }
        }
        ...
        if(_master)
            _theme = _master.Theme;
        par.calculateTextTheme2(_theme, _slide, _layout, _master, null/*fontRef*/);
    },

    canIncreaseIndent: function(bIncrease)
    {
        return this.Content.canIncreaseIndent(bIncrease);
    },

    RecalculateContent: function()
    {
        this.Content.Recalculate();
        this.Row.Table.Parent.RecalculateSize();
        this.Row.Table.Parent.RecalculateCurPos();
    },
    Set_Id : function(newId)
    {
        g_oTableId.Reset_Id( this, newId, this.Id );
    ...
    getStylesForParagraph: function(level)
    {
        var _styles = this.Row.Table.getStylesForParagraph(level);
        var _table = this.Row.Table;
        if(_table.styleIndex !== -1)
        {
            var _graphic_frame = _table.Parent;
            var _global_table_style, _presentation = null, _theme = null, _master = null, _layout = null, _slide = null;
            switch(_graphic_frame.parent.kind)
            {
                case SLIDE_KIND:
                {
                    ...
                    _layout = _slide.Layout;
                    _master = _layout.Master;
                    _presentation = _master.presentation;
                    _theme = _master.Theme;
                    break;
                }
                case LAYOUT_KIND:
            ...
            _global_table_style = _presentation.globalTableStyles[_table.styleIndex];
            var _table_cell_style = new CStyle("tableStyle", null, null, null);
            _table_cell_style.ParaPr = _global_table_style.TableWholeTable.ParaPr.Copy();
            _table_cell_style.TextPr = _global_table_style.TableWholeTable.TextPr.Copy();

            var _table_look = _table.Get_TableLook();
            var _row_index = this.Row.Index;
            var _cell_index = this.Index;


            var _table_pr = _table.Get_CompiledPr(false);

            if ( true === _table_look.Is_BandVer() && !( (true === _table_look.Is_LastCol() && this.Row.Get_CellsCount() - 1 === _cell_index) || (true === _table_look.Is_FirstCol() && 0 === _cell_index) ) )
            {
                var ColBandSize = _table_pr.TablePr.TableStyleColBandSize;
                var _ColIndex   = ( true != _table_look.Is_FirstCol() ? _cell_index : _cell_index - 1 );
                var ColIndex = ( 1 != ColBandSize ? Math.floor( _ColIndex / ColBandSize ) : _ColIndex );
                var TableBandStyle = null;
                if ( 0 === ColIndex % 2 )
                    TableBandStyle = _global_table_style.TableBand1Vert;
                else
                    TableBandStyle = _global_table_style.TableBand2Vert;

                _table_cell_style.ParaPr.Merge( TableBandStyle.ParaPr );
                _table_cell_style.TextPr.Merge( TableBandStyle.TextPr );
            }

            // Совместим настройки с настройками для групп строк
            if ( true === _table_look.Is_BandHor() )
            {
                var RowBandSize = _table_pr.TablePr.TableStyleRowBandSize;
                var __RowIndex  = ( true != _table_look.Is_FirstRow() ? _row_index : _row_index - 1 );
                var _RowIndex = ( 1 != RowBandSize ? Math.floor( __RowIndex / RowBandSize ) : __RowIndex );
                var TableBandStyle = null;
                if ( 0 === _RowIndex % 2 )
                    TableBandStyle = _table_pr.TableBand1Horz;
                else
                    TableBandStyle = _table_pr.TableBand2Horz;
            ...
            if ( true === _table_look.Is_LastCol() && this.Row.Get_CellsCount() - 1 === _cell_index )
            {
                _table_cell_style.ParaPr.Merge(_global_table_style.TableLastCol.ParaPr);
                _table_cell_style.TextPr.Merge(_global_table_style.TableLastCol.TextPr);
            }

            // Совместим настройки с настройками для первой колонки
            if ( true === _table_look.Is_FirstCol() && 0 === _cell_index )
            {
                _table_cell_style.ParaPr.Merge(_global_table_style.TableFirstCol.ParaPr);
                _table_cell_style.TextPr.Merge(_global_table_style.TableFirstCol.TextPr);
            }

            // Совместим настройки с настройками для последней строки
            if ( true === _table_look.Is_LastRow() && _table.Content.length - 1 === _row_index )
            {
                _table_cell_style.ParaPr.Merge( _global_table_style.TableLastRow.ParaPr );
                _table_cell_style.TextPr.Merge( _global_table_style.TableLastRow.TextPr );
            }

            // Совместим настройки с настройками для первой строки
            if ( true === _table_look.Is_FirstRow() && 0 === _row_index )
            {
                 _table_cell_style.TextPr.Merge( _global_table_style.TableFirstRow.TextPr );
                 _table_cell_style.ParaPr.Merge( _global_table_style.TableFirstRow.ParaPr );
            }

            // Совместим настройки с настройками для правой нижней ячейки
            if ( this.Row.Get_CellsCount() - 1 === _cell_index && _table.Content.length - 1 === _row_index )
            {
               _table_cell_style.TextPr.Merge(_global_table_style.TableBRCell.TextPr );
               _table_cell_style.ParaPr.Merge(_global_table_style.TableBRCell.ParaPr );
            }

            // Совместим настройки с настройками для левой нижней ячейки
            if ( 0 === _cell_index && _table.Content.length - 1 === _row_index )
            {
                _table_cell_style.TextPr.Merge(_global_table_style.TableBLCell.TextPr );
                _table_cell_style.ParaPr.Merge(_global_table_style.TableBLCell.ParaPr );
            }

            // Совместим настройки с настройками для правой верхней ячейки
            if ( this.Row.Get_CellsCount() - 1 === _cell_index && 0 === _row_index )
            {
                _table_cell_style.TextPr.Merge(_global_table_style.TableTRCell.TextPr );
                _table_cell_style.ParaPr.Merge(_global_table_style.TableTRCell.ParaPr );
            }

            // Совместим настройки с настройками для левой верхней ячейки
            if ( 0 === _cell_index && 0 === _row_index )
            {
                _table_cell_style.TextPr.Merge(_global_table_style.TableTLCell.TextPr );
                _table_cell_style.ParaPr.Merge(_global_table_style.TableTLCell.ParaPr );
            }

            _styles.Style[_styles.Id] = _table_cell_style;
            _table_cell_style.BasedOn = _styles.Id-1;
            ++_styles.Id;
        }
        return _styles;
    },
...
//-----------------------------------------------------------------------------------
    Recalc_CompiledPr : function()
    {
        this.CompiledPr.NeedRecalc = true;
        var _style_index;
        for(_style_index = 0; _style_index < 9; ++_style_index)
        {
            this.Content.arrStyles[_style_index] = null;
        }
    ...
    Internal_Compile_Pr : function()
    {
        ...
        if(CellPr  !== null && typeof CellPr  === "object" )
        {
            if(CellPr.Shd  !== null && typeof CellPr.Shd  === "object")
            {
                ...
                if( CellPr.Shd.unifill !== null && typeof CellPr.Shd.unifill === "object"
                    && CellPr.Shd.unifill.fill !== null && typeof CellPr.Shd.unifill.fill === "object")
                {
...
//-----------------------------------------------------------------------------------
    ...
    {
        var _table = this.Row.Table;
        var _table_style_index = _table.styleIndex;
        if(typeof _table_style_index === "number" && _table_style_index !== -1)
        {
            var _graphic_frame = _table.Parent;
            var _presentation;
            switch(_graphic_frame.parent)
            {
                case SLIDE_KIND:
                {
                    _presentation
        ...
        //return this.Row.Table.Parent.Get_TableStyleForPara(this.Row.Table.styleIndex);
    ...
    {
        return this.Row.Table.Get_PageContentStartPos( PageNum, this.Row.Index, this.Index, true );
    },

    Add_InlineObjectXY : function( Drawing, X, Y, PageNum )
    {
        return this.Row.Table.Parent.Add_InlineObjectXY( Drawing, X, Y, PageNum );
    },

    Add_InlineTableXY : function(Table, X, Y, PageNum_Abs)
    {
        return this.Row.Table.Parent.Add_InlineTableXY( Table, X, Y, PageNum_Abs );
    },
    ...
    Set_CurrentElement : function()
    {
        ...
        // Делаем данную ячейку текущей в таблицу
...
//-----------------------------------------------------------------------------------
    ...
    Recalculate : function()
    {
        this.Content.Recalculate(false, undefined, true);
    },
...
//-----------------------------------------------------------------------------------
    ...
    Copy_Pr : function(OtherPr)
    {
        ...
        else
        {
            var Shd_new =
            {
                ...
                unifill: OtherPr.Shd.unifill,
                fillRef: OtherPr.Shd.fillRef
            };
    ...
    Get_Border : function(Type)
    {
        ...
        switch (Type)
        {
            case 0 :
            {
                if ( null != Borders.Top && Borders.Top.Value != border_None )
                    Border = Borders.Top;
                else
                {
            ...
            case 1 :
            {
                if ( null != Borders.Right && Borders.Right.Value != border_None )
            ...
            case 2 :
            {
                if ( null != Borders.Bottom && Borders.Bottom.Value != border_None )
            ...
            case 3 :
            {
                if ( null != Borders.Left && Borders.Left.Value != border_None )
                    Border = Borders.Left;
                else
                {
...
//-----------------------------------------------------------------------------------
    Undo : function(Data)
    {
        ...
        History.RecalcData_Add( this.Row.Table );
    },
        ...
        switch ( Type )
        {
            ...
            case historyitem_TableCell_Margins:
            {
                ...
                else
                {
                    ...
                    {
                        this.Pr.TableCellMar.Top    = new CTableMeasurement(tblwidth_Auto, 0);
                        this.Pr.TableCellMar.Left   = new CTableMeasurement(tblwidth_Auto, 0);
                        this.Pr.TableCellMar.Bottom = new CTableMeasurement(tblwidth_Auto, 0);
                        this.Pr.TableCellMar.Right  = new CTableMeasurement(tblwidth_Auto, 0);
