...
function CheckLicense(licenseUrl, companyId, userId, callback) {
  ...
  g_fGetJSZipUtils().getBinaryContent(licenseUrl, function(err, data) {
    ...
    try {
      ...
      callback(false, isValid ? CheckUserInLicense(companyId, userId, oLicense) : false);
    } catch(e) {
      callback(true, false);
    }
  });
 ...
 * @returns {boolean}
 */
function CheckUserInLicense(companyId, userId, oLicense) {
  var res = false;
  var superuser = 'onlyoffice';
  try {
    if (oLicense.users) {
      if ((companyId === oLicense['company_id'] && oLicense.users.hasOwnProperty(userId)) ||
        (superuser === oLicense['company_id'] && oLicense.users.hasOwnProperty(userId = superuser))) {
