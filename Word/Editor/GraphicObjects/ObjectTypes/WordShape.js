...
WordShape.prototype =
{
    ...
    Check_TableCoincidence: function( Table )
    {
        ...
        else
        {
            ...
            if(isRealObject(cur_group.parent)
                && isRealObject(cur_group.parent.Parent)
                && isRealObject(cur_group.parent.Parent.Parent))
            {
    ...
    Set_CurrentElement: function()
    {
        ...
        {
            if(isRealObject(this.parent)
                && isRealObject(this.parent.Parent)
                && isRealObject(this.parent.Parent.Parent))
            {
    ...
    Get_PageContentStartPos: function(pageNum)
    {
        if(this.textBoxContent)
        {
            ...
            {
                return {X: 0, Y: 0, XLimit: this.absExtX, YLimit: 20000};
            }
        }
        return null;
    },
    ...
    OnContentRecalculate: function()
    {
        /*if(this.bodyPr.anchor !== VERTICAL_ANCHOR_TYPE_TOP)
        {
            ...
            {
                if(isRealObject(arr_graphic_objects[i]) && isRealObject(arr_graphic_objects[i].GraphicObj) && isRealObject(arr_graphic_objects[i].GraphicObj.transform))
                global_MatrixTransformer.MultiplyAppend(arr_graphic_objects[i].transform, this.transformText);
            }
        }    */
    },
    ...
    setTextVerticalAlign: function(align)
    {
        if(this.bodyPr.anchor === align)
            return;
        History.Add(this, {Type: historyitem_SetVerticalShapeAlign, oldAlign: this.bodyPr.anchor, newAlign: align});
    ...
    hitToPath: function(x, y)
    {
        if(isRealObject(this.spPr.geometry) && typeof this.spPr.geometry.hitInPath === "function")
        {
           ...
           // var _transformed_point = this.transformPointRelativeShape(tx, ty);
    ...
    updateSelectionState: function()
    {
        if(this.textBoxContent)
        {
         ...
         //   this.document.DrawingDocument.UpdateTargetTransform(this.nullTransform);
        }
    },
    ...
    Load_Changes : function(reader)
    {
        ...
        switch(type)
        {
            ...
            case historyitem_SetSpPr:
            {
                ...
                this.spPr = sp_pr;
                if(isRealObject(this.spPr.Fill)
                    && isRealObject(this.spPr.Fill.fill)
...
//-----------------------------------------------------------------------------------
    ...
    readFromBinaryForCopyPaste: function(r, bNoRecalc)
    {
        ...
        this.setSpPr(sp_pr);
        /*if(isRealObject(this.spPr.Fill) && isRealObject(this.spPr.Fill.fill)
            && this.spPr.Fill.fill instanceof CBlipFill
            && typeof this.spPr.Fill.fill.RasterImageId === "string")
        {
            editor.WordControl.m_oLogicDocument.DrawingObjects.urlMap.push(this.spPr.Fill.fill.RasterImageId);
        }*/
    ...
    {
        History.Add(this, {Type: historyitem_SetTextBoxContent, contentId: content.Get_Id()});
