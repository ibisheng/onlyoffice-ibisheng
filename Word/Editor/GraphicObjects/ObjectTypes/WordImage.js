...
function WordImage(parent/*(WordGraphicObject)*/, document, drawingDocument, group)
{
    ...
    this.transform = null;//new CMatrix();
...
WordImage.prototype =
{
    ...
    {
        var data = {Type:historyitem_ChangeDiagram, oldDiagram: this.chart == null ? null : this.chart.Get_Id(), newDiagram: chart.Get_Id()};
    ...
    init: function(Img, W, H, chart, canvas)
    {
        ...
        {
            var data = {Type:historyitem_ChangeDiagram2, diagram: chart.Get_Id(), W: W, H: H};
            History.Add(this, data);
            this.chart = chart;
            this.setRasterImage(chart.img);
        }
    ...
    calculateTransformChildElements: function()
    {
        ...
        this.absRot = 0;        //TODO:!!!!!!!!!!!!!
        this.absFlipH = false;  //TODO:!!!!!!!!!!!!!
        this.absFlipV = false;  //TODO:!!!!!!!!!!!!!
       ...
       /* if(isRealObject(this.parent))
        {
            ...
            else
            {
                ...
                para_drawing.OnEnd_ChangeFlow(bounds.l, bounds.t, para_drawing.pageIndex, bounds.r - bounds.l, bounds.b - bounds.t,
            ...
            var _image = editor.ImageLoader.LoadImage(_getFullImageSrc(diagramm.img), 1);
            if (null != _image)
            {
                this.drawingDocument.OnRecalculatePage(this.pageIndex, this.document.Pages[this.pageIndex]);
            ...
            else
            {
                ...
                editor.asyncImageEndLoaded2 = function()
                {
                    editor.WordControl.m_oLogicDocument.DrawingDocument.OnRecalculatePage(page_index, editor.WordControl.m_oLogicDocument.Pages[page_index]);
                    editor.WordControl.m_oLogicDocument.DrawingDocument.OnEndRecalculate(true, false);
    ...
    calculate: function()//расчет при открытии
    {
        if(this.group !== null)
        {
    ...
    getAngle: function(x, y)
    {
        ...
        var tx = result_x-x_lt, ty = result_y-y_lt;
    ...
    hit: function(x, y)
    {
        if(isRealObject(this.parent) && isRealObject(this.parent.Parent) && isRealObject(this.parent.Parent.Parent)
            && this.parent.Parent.Parent instanceof CDocumentContent
            && this.parent.wrappingType !== WRAPPING_TYPE_NONE
            && !(this.parent.Parent.Parent.Is_HdrFtr() && this.parent.Parent.Parent.Is_TopDocument())
            && !isRealObject(this.group))
        {
            var tx, ty;
            var b_check = false;
    ...
    {
        if(this.spPr.geometry !== null && typeof this.spPr.geometry === "object"
            && typeof this.spPr.geometry.hitToPath === "function")
        {
    ...
    updateSelectionState: function()
    {
        if(this.textBoxContent)
        {
            ...
            if ( true === Doc.Is_SelectionUse() && !Doc.Selection_IsEmpty()) {
                // this.Container.DrawingDocument.UpdateTargetTransform(this.transformText);
    ...
    setPageIndex: function(newPageIndex)
    {
        ...
        //this.selectStartPage = newPageIndex;
    },
    ...
    calculateWrapPolygon: function()
    {
        if(this.spPr.geometry === null)
        {
            var ret = [];
            ret.push({x: 0, y: 0});
            ret.push({x: this.absExtX, y: 0});
            ret.push({x: this.absExtX, y: this.absExtY});
            ret.push({x: 0, y: this.absExtY});
            return ret;
        }
    ...
    readFromBinaryForCopyPaste: function(r, bNoRecalc)
    {
        ...
        if(r.GetBool())
        {
           ...
           /* if(typeof this.blipFill.RasterImageId === "string")
                editor.WordControl.m_oLogicDocument.DrawingObjects.urlMap.push(this.blipFill.RasterImageId);   */
        }
        if(r.GetBool())
        {
