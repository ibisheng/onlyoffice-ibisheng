function WordGroupShapes(parent, document, drawingDocument, group)
{
    ...
    this.textBoxContent = null;     //new CDocumentContent

    this.group = (group !== null && typeof group === "object") ? group : null;
    this.mainGroup = null;// расчитывается в calculate

    this.spTree = [];//массив без групп
    this.arrGraphicObjects = [];//массив согласно структуре
...
WordGroupShapes.prototype =
{
    ...
    normalizeXfrm: function()
    {
        ...
        for(var i = 0; i < this.arrGraphicObjects.length; ++i)
        {
            ...
            /*else
            {
                cur_xfrm.extX *= scaleY;
                cur_xfrm.extY *= scaleX;
            }*/
    ...
    /*calculateTransformMatrixInGroup: function()
     {
     var _transform = new CMatrix();
    ...
    {
        return {extX: this.absExtX, extY: this.absExtY};
    },
    ...
    getAngle: function(x, y)
    {
        ...
        /* this.calculateLeftTopPoint();
         ...
         var _angle = Math.PI*0.5 + Math.atan2(_relative_x - this.absExtX*0.5, _relative_y - this.absExtY*0.5);
         return !this.absFlipV ? _angle : _angle + Math.PI;     */
    },
    ...
    createTrackObjectForMove: function(majorOffsetX, majorOffsetY)
    {
        ...
        //return new MoveTrackGroup(this, majorOffsetX, majorOffsetY);
    },

    createTrackObjectForResize: function(handleNumber, pageIndex)
    {
        return new GroupForResize(this, null, handleNumber);
        //return new ResizeTrackGroup(this, handleNumber, pageIndex);
    },
    ...
    recalculateAfterInternalResize: function()
    {
        ...
        var _xc, _yc;
        if((_cur_rot >= 0 && _cur_rot < Math.PI*0.25)
            || (_cur_rot > 3*Math.PI*0.25 && _cur_rot < 5*Math.PI*0.25)
            || (_cur_rot > 7*Math.PI*0.25 && _cur_rot < 2*Math.PI))
        {
            _x_min = _xfrm.offX;
            _y_min = _xfrm.offY;
    ...
    {
        History.Add(this, {Type: historyitem_AddGraphicObject, id: graphicObject.Get_Id(), index: this.arrGraphicObjects.length});
        this.arrGraphicObjects.push(graphicObject);
    },

    addToSpTree: function(sp)
    {
        History.Add(this, {Type: historyitem_AddToSpTree, id: sp.Get_Id(), index: this.spTree.length});
        this.spTree.push(sp);
    },
    ...
    {
        return {x: this.absOffsetX, y: this.absOffsetY};
    },
    ...
    drawSelect: function()
    {
        ...
        for(var _selection_index = 0; _selection_index < _selection_count; ++_selection_index)
        {
            ...
            this.drawingDocument.DrawTrack(TYPE_TRACK_SHAPE , _graphic_object.transform, 0, 0, _graphic_object.absExtX, _graphic_object.absExtY, /*shape.geometry ? shape.geometry.preset == "line"  : false*/false);
        }
    ...
    calculateFullTransformMatrix: function()
    {
        ...
        if(this.absFlipH)///TODO
        {
            global_MatrixTransformer.ScaleAppend(_transform, -1, 1);
        }
        if(this.absFlipV)//TODO
        {
            global_MatrixTransformer.ScaleAppend(_transform, 1, -1);
        }
            ...
            && this.parent.Parent.Parent instanceof CDocumentContent && this.parent.wrappingType !== WRAPPING_TYPE_NONE && !(this.parent.Parent.Parent.Is_HdrFtr() && this.parent.Parent.Parent.Is_TopDocument()))
        {
            ...
            if(b_check)
            {
                ...
                if(cur_doc_content.Parent instanceof WordShape)
                {
                    ...
                    {
                       // rel_page_index = cur_doc_content.Parent.Row.Table.PageNum;
    ...
    hitInBox: function(x, y)
    {
        ...
        return (HitInLine(_hit_context, _tr_x, _tr_y, 0, 0, this.absExtX, 0) ||
            HitInLine(_hit_context, _tr_x, _tr_y, this.absExtX, 0, this.absExtX, this.absExtY)||
            HitInLine(_hit_context, _tr_x, _tr_y, this.absExtX, this.absExtY, 0, this.absExtY)||
            HitInLine(_hit_context, _tr_x, _tr_y, 0, this.absExtY, 0, 0) ||
            HitInLine(_hit_context, _tr_x, _tr_y, this.absExtX*0.5, 0, this.absExtX*0.5, -this.drawingDocument.GetMMPerDot(TRACK_DISTANCE_ROTATE)));
    },
    ...
    hitToTextRect: function(x, y)
    {
        for(var object_index = 0;  object_index < this.spTree.length; ++object_index)
        {
            if(this.spTree[object_index].hitToTextRect(x, y))
                return {hit:true, num: object_index};
        }
        return {hit: false};
    },
    ...
    removeObjectFromSpTreeById: function(id)
    {
        ...
        for(var i = 0; i < sp_tree.length; ++i)
        {
            ...
            {
                var data = {Type: historyitem_RemoveFromSpTree, id: id};
                History.Add(this, data);
                sp_tree.splice(i, 1);
                return;
            }
    ...
    removeObjectFromArrGraphicObjects: function(id)
    {
        ...
        for(var i = 0; i < arr_gr_objects.length; ++i)
        {
            ...
            {
                var data = {Type: historyitem_RemoveFromArrGraphicObj, id: id, index: i};
                History.Add(this, data);
        ...
        if(isRealObject(new_obj))
        {
            for(var i = 0; i < this.arrGraphicObjects.length; ++i)
            {
                if(this.arrGraphicObjects[i].Get_Id() === oldId)
                {
                    ...
                    History.Add(this, {Type: historyitem_SwapGrObject, oldId: oldId, newId: newId});
                    return;
                }
            }
        }
    ...
    removeObjectFromArrGraphicObjects2: function(index)
    {
        ...
        {
            var data = {Type: historyitem_RemoveFromArrGraphicObj2, id: arr_gr_objects[index].Get_Id(), index: index};
            History.Add(this, data);
        ...
        switch(data.Type)
        {
            ...
            case historyitem_RemoveFromSpTree:
            {
                ...
                if(isRealObject(sp))
                {
                    ...
                    {
                        if(isRealObject(a) && isRealObject(a.parent)
                            && isRealObject(b) && isRealObject(b.parent))
    ...
    {
        History.Add(this, {Type:historyitem_SetBodyPr, bodyPr: b});
        this.bodyPr = b;
    },
    ...
    {
        History.Add(this, {Type: historyitem_UpadteSpTreeBefore});
        var gr_objects = this.arrGraphicObjects;
        ...
        for(var i = 0; i < gr_objects.length; ++i)
        {
            ...
            {
                History.Add(this, {Type: historyitem_MoveShapeInArray, oldPos: i, newPos: new_pos});
        ...
        this.updateSpTree();
        History.Add(this, {Type: historyitem_UpadteSpTreeAfter});
    },
    ...
    bringForward: function()
    {
        ...
        for(var i = gr_objects.length - 1; i > -1 ; --i)
        {
            ...
            {
                History.Add(this, {Type: historyitem_MoveShapeInArray, oldPos: i, newPos: i+1});
    ...
    sendToBack: function()
    {
        ...
        if(typeof last_id === "string")
        {
            ...
            for(i = gr_objects.length-1; i > -1 ; --i)
            {
                ...
                if(gr_objects[i].selected)
                {
                    History.Add(this, {Type: historyitem_MoveShapeInArray, oldPos: i, newPos: 0});
    ...
    {
        History.RecalcData_Add( { Type : historyrecalctype_Flow, Data : this});
    },
