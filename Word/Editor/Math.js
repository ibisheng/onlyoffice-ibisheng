...
CParaMathLineWidths.prototype.UpdateWidth = function(Line, W)
{
    ...
    var bFastUpdate = CurLine >= this.Widths.length;
    ...
    if(CurLine !== 0)
    {
        if(bFastUpdate)
        {
            bUpdMaxWidth = this.UpdateMinMax(CurLine);
        }
        else if(bNeedUpdate)
        {
            ...
            if(lng > 1)
            {
                this.MaxW = this.Widths[1];
                this.MinW = this.Widths[1];
