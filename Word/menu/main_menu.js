		...
		for (var i = 0; i < arg.length;i++){
			...
			if ( elemArg != undefined )
			{
				...
				if ( ObjectType == c_oAscTypeSelectElement.Image)
                {
                    ...
                    Image_OnWrapChange(elemVal.get_WrappingStyle());
                    Image_OnCanBeFlow(elemVal.get_CanBeFlow());

                    if ( true === elemVal.get_AllowOverlap() )
                    {
                        document.getElementById("imgAllowOverlap").checked = true;
                    }
                    else
                        document.getElementById("imgAllowOverlap").checked = false;
				...
				else if ( ObjectType == c_oAscTypeSelectElement.Table)
                {
                    ...
                    if ( true === elemVal.get_AllowOverlap() )
                        document.getElementById( "tblAllowOverlap").checked = true;
                    else
                        document.getElementById( "tblAllowOverlap").checked = false;
                    ...
                    if ( undefined != elemVal.TableStyle )
                        CurStyle = elemVal.TableStyle;

                    var Styles = editor.WordControl.m_oLogicDocument.Styles;
                    var TableStyles = Styles.Get_AllTableStyles();

                    var TableStyleSelect = document.getElementById("tblStyle");
                    TableStyleSelect.options.length = 0;
                    for ( var TblIndex = 0; TblIndex < TableStyles.length; TblIndex++ )
                    {
                        var StyleName = Styles.Get_Name( TableStyles[TblIndex] );
                        $('#tblStyle').append($('<option>', { TblIndex : StyleName }).text(StyleName));

                        if ( CurStyle === TableStyles[TblIndex] )
                            TableStyleSelect.selectedIndex = TblIndex;
                    }
                    ...
                    if ( undefined != elemVal.TableLook )
                    {
                        ...
                        document.getElementById("tblLookHorBand").checked  = elemVal.TableLook.BandVer;
                        document.getElementById("tblLookVerBand").checked  = elemVal.TableLook.BandHor;
                    }
